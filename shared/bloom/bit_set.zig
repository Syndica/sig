...
pub fn DynamicArrayBitSet(comptime MaskIntType: type) type {
    ...
    if (mask_info != .int) {
        @compileError(
            "ArrayBitSet can only operate on integer masks, but was passed " ++
                @typeName(MaskIntType),
        );
    }

    // It must also be unsigned.
    if (mask_info.int.signedness != .unsigned) {
        @compileError(
    ...
    if (!std.math.isPowerOfTwo(@bitSizeOf(MaskIntType))) {
        ...
        @compileError(
            "ArrayBitSet was passed integer type " ++ @typeName(MaskIntType) ++
                ", which is not a power of two.  Please round this up to a power of two integer " ++
                "size (i.e. " ++
                @typeName(FixedMaskType) ++ ").",
        );
    }
