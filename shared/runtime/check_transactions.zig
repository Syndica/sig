...
const SYSTEM_PROGRAM_ID: Pubkey = .parse("11111111111111111111111111111111");
const ED25519_PRECOMPILE_ID: Pubkey = .parse("Ed25519SigVerify111111111111111111111111111");
const SECP256K1_PRECOMPILE_ID: Pubkey = .parse("KeccakSecp256k11111111111111111111111111111");
const SECP256R1_PRECOMPILE_ID: Pubkey = .parse("Secp256r1SigVerify1111111111111111111111111");
const ADVANCE_NONCE_ACCOUNT_INSTRUCTION_DATA = [_]u8{ 4, 0, 0, 0 };
...
pub const SignatureCounts = struct {
    ...
    pub fn fromTransaction(transaction: *const RuntimeTransaction) SignatureCounts {
        return .{
            .num_ed25519_signatures = sumPrecompileSigs(transaction, &ED25519_PRECOMPILE_ID),
            .num_secp256k1_signatures = sumPrecompileSigs(transaction, &SECP256K1_PRECOMPILE_ID),
            .num_secp256r1_signatures = sumPrecompileSigs(transaction, &SECP256R1_PRECOMPILE_ID),
...
fn getSystemAccountKind(account: *const AccountSharedData) ?SystemAccountKind {
    if (!account.owner.equals(&SYSTEM_PROGRAM_ID)) return null;
...
pub fn getDurableNonce(
    ...
    if (!program_key.equals(&SYSTEM_PROGRAM_ID)) return null;
...
test "checkAge: nonce account" {
    ...
    const instruction_data = try allocator.dupe(
        u8,
        &ADVANCE_NONCE_ACCOUNT_INSTRUCTION_DATA,
    );
    ...
    const transaction = RuntimeTransaction{
        ...
        .recent_blockhash = recent_blockhash,
        .instructions = &.{.{
            .program_meta = .{ .pubkey = SYSTEM_PROGRAM_ID, .index_in_transaction = 0 },
            .account_metas = metas,
            .dedupe_map = blk: {
