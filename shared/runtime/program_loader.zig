...
const loaded_accounts = @import("loaded_accounts.zig");
const AccountLoadError = loaded_accounts.AccountLoadError;
const wrapDB = loaded_accounts.wrapDB;
...
pub fn testLoad(
    ...
    const TestAccountReader = struct {
        ...
        const LoadedTestAccount = struct {
            ...
            const Data = struct {
                ...
                pub fn read(self: Data, start: u64, dest: []u8) usize {
                    const offset: usize = @intCast(start);
                    const end = @min(offset + dest.len, self.bytes.len);
                    const slice = self.bytes[offset..end];
            ...
            pub fn deinit(self: LoadedTestAccount, alloc: std.mem.Allocator) void {
        ...
        pub fn get(
            ...
            pubkey: Pubkey,
        ) std.mem.Allocator.Error!?LoadedTestAccount {
            const account = self.account_shared_data_map.get(pubkey) orelse return null;
            return .{
                .lamports = account.lamports,
                .data = .{ .bytes = try alloc.dupe(u8, account.data) },
                .owner = account.owner,
    ...
    for (accounts.keys(), accounts.values()) |address, account| {
        try loadIfProgram(
            ...
            address,
            &account,
            TestAccountReader{ .account_shared_data_map = accounts },
            environment,
            slot,
        );
    }
