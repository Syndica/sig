...
pub const Error = runtime.vm.ExecutionError;
...
test findProgramAddress {
    ...
    const cost = tc.compute_budget.create_program_address_units;
    const address = runtime.program.bpf_loader.v2.ID;
...
test createProgramAddress {
    ...
    const cost = tc.compute_budget.create_program_address_units;
    const address = runtime.program.bpf_loader.v3.ID;
...
test getProcessedSiblingInstruction {
    ...
    for (&account_params) |*a| a.* = .{
        .pubkey = Pubkey.initRandom(prng.random()),
        .owner = runtime.program.bpf_loader.v2.ID,
    };
...
test "set and get return data" {
    ...
    const program_id = runtime.program.bpf_loader.v2.ID;
    const instr_info = runtime.InstructionInfo{
        .program_meta = .{
            .index_in_transaction = 0,
            .pubkey = program_id,
        },
    ...
    try runtime.executor.pushInstruction(&tc, instr_info);
