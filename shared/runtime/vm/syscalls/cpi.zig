...
const serialize = runtime.program.bpf.serialize;
...
const PRECOMPILES = runtime.program.precompiles.PRECOMPILES;

const SyscallError = runtime.vm.syscalls.Error;
const RegisterMap = runtime.vm.interpreter.RegisterMap;
const Error = runtime.vm.ExecutionError;
...
pub fn invokeSigned(AccountInfo: type) runtime.vm.SyscallFn {
    return struct {
        fn common(
            ...
            const info = try runtime.executor.prepareCpiInstructionInfo(
                tc,
                instruction,
                signers.slice(),
            );
            ...
            try runtime.executor.executeInstruction(allocator, ic.tc, info);
...
const TestContext = struct {
    cache: sig.utils.collections.PubkeyMap(runtime.AccountSharedData),
    tc: *TransactionContext,
    ...
    fn init(allocator: std.mem.Allocator, prng: std.Random, account_data: []const u8) !TestContext {
        ...
        try runtime.executor.pushInstruction(tc, try testing.createInstructionInfo(
            tc,
            system_program.ID,
    ...
    fn deinit(self: *TestContext, allocator: std.mem.Allocator) void {
        ...
        runtime.testing.deinitAccountMap(self.cache, allocator);
    }
