    ...
    return struct {
        ...
        pub const Context = extern struct {
            ...
            const contract: Transcript.Contract = &.{
                .{ .label = "commitments", .type = .bytes },
                .{ .label = "bit-lengths", .type = .bytes },
            };
        ...
        pub fn verify(self: Self) !void {
            ...
            for (bit_lengths.constSlice()) |bit_length| {
                if (bit_length == 0 or bit_length > MAX_SINGLE_BIT_LENGTH)
                    return error.IllegalAmountBitLength;
                batched_bit_length = try std.math.add(u64, batched_bit_length, bit_length);
            }
...
test "non power of two length" {
    ...
    const proof_data = try Data(32).init(
...
test "non power of two length too large" {
    const amount_1: u64 = std.math.maxInt(u10);
    const amount_2: u64 = std.math.maxInt(u22);
    ...
    const proof_data = try Data(32).init(
        &.{ commitment_1, commitment_2 },
        &.{ amount_1, amount_2 },
        &.{ 12, 20 }, // The second amount will not be able to fit.
        &.{ opening_1, opening_2 },
    );
