...
///
/// Mirrors agave's predicate, which AND-combines five SIMD-0387 / SIMD-0185 /
/// SIMD-0232 feature gates. `commission_rate_in_basis_points` is owned by a
/// separate in-flight PR on Sig; this gate currently checks the four feature
/// flags that exist on Sig today and a TODO marks the 5th. The omission is
/// safe in practice because all five flags are `.unsupported` on Sig and
/// cannot be activated outside of explicit test/fuzz setups, and the V4 state
/// layout already stores commission as raw u16 basis-points regardless of
/// whether SIMD-0291's UpdateCommissionBps instruction is wired up.
///
/// TODO(simd-0291): re-introduce the
/// `fs.active(.commission_rate_in_basis_points, slot)` check once that
/// feature lands on Sig.
///
/// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_processor.rs#L63-L70
...
///
/// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L876-L905
/// [agave] read_new_collector_account: https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_processor.rs#L83-L95
...
///
/// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_processor.rs#L334-L361
/// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L1142-L1188
...
fn authorize(
    ...
    // [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L703-L772
    ...
    switch (vote_authorize) {
        ...
        .voter_with_bls => |args| {
            ...
            // [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L735-L770
...
///
/// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L1047-L1063
...
// would succeed.
//
// [agave] https://github.com/anza-xyz/agave/blob/master/programs/vote/src/vote_state/mod.rs#L705-L709
...
// Test vector taken from firedancer's POP success case 1 / Sig's
// `proofOfPossessionVerify` test in src/crypto/bls12_381/lib.zig. The vote
// account pubkey must be exactly the middle 32 bytes of the POP message,
// otherwise PoP verification fails.
//
...
fn alpenglowPopFixture() struct {
    vote_pubkey: Pubkey,
    bls_pubkey: [48]u8,
    bls_proof: [96]u8,
} {
    var vote_pk_bytes: [32]u8 = undefined;
    _ = std.fmt.hexToBytes(
        &vote_pk_bytes,
        "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
    ) catch unreachable;
    var bls_pk: [48]u8 = undefined;
    _ = std.fmt.hexToBytes(
        &bls_pk,
    ...
    return .{
        .vote_pubkey = .{ .data = vote_pk_bytes },
        .bls_pubkey = bls_pk,
        .bls_proof = proof,
    };
}

// [SIMD-0387] initialize_account_v2 success path: all five (currently four,
// pending `commission_rate_in_basis_points`) co-dep features active, a valid
// BLS PoP, and both commission collectors set to the vote account itself
// (the SIMD-0232/SIMD-0464 escape hatch). The resulting VoteStateV4 must
// carry the BLS pubkey, basis-points commissions and collectors derived from
// the VoteInitV2 payload.
test "vote_program: initialize_account_v2 success (escape-hatch collectors)" {
    ...
    const fixture = alpenglowPopFixture();
