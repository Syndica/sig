...
pub const LtHash = struct {
    ...
    pub fn init(hasher: *const std.crypto.hash.Blake3) LtHash {
        var self: LtHash = .{ .data = undefined };
        hasher.final(self.bytes());
        return self;
    }

    pub fn initRandom(random: std.Random) LtHash {
        var self = LtHash.IDENTITY;
        random.bytes(self.bytes());
        return self;
    }
...
test "hello and world hash correctly" {
    const expected_hello_lt_hash = LtHash{ .data = .{
    ...
    const expected_world_lt_hash = LtHash{ .data = .{
    ...
    inline for (
        .{ "hello", "world!" },
        .{ expected_hello_lt_hash, expected_world_lt_hash },
    ) |input, expected| {
        var hasher = std.crypto.hash.Blake3.init(.{});
        hasher.update(input);
        const actual = LtHash.init(&hasher);
