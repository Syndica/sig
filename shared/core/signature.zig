...
pub const Signature = extern struct {
    ...
    pub fn verify(self: Signature, pubkey: Pubkey, message: []const u8) !void {
        const public_key = try std.crypto.sign.Ed25519.PublicKey.fromBytes(pubkey.data);
        try std.crypto.sign.Ed25519.Signature.fromBytes(self.toBytes()).verify(message, public_key);
    }
