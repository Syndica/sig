...
if [ "$($ZIG version)" != "0.15.2" ]; then
    echo "Need Zig 0.15.2 to compile these tests"
    exit 1
fi
...
LD_V0="${LD_FLAGS} --script v2/components/runtime/data/test-elfs/elf_sbpfv0.ld"
LD_V2="${LD_FLAGS} -Bsymbolic --script v2/components/runtime/data/test-elfs/elf.ld"
...
for ZIG_FILE in v2/components/runtime/data/test-elfs/*.zig; do
    BASE_NAME=$(basename "$ZIG_FILE" .zig)
    
    $ZIG build-obj "$ZIG_FILE" -target bpfel-freestanding -OReleaseSmall -fstrip -fno-emit-bin -femit-llvm-bc="v2/components/runtime/data/test-elfs/${BASE_NAME}.bc"
    if [[ ! " ${EXCLUDE_V2[@]} " =~ " ${BASE_NAME} " ]]; then
        $CC_V2 "v2/components/runtime/data/test-elfs/${BASE_NAME}.bc" -c -o "v2/components/runtime/data/test-elfs/${BASE_NAME}.o"
        $LD_V2 "v2/components/runtime/data/test-elfs/${BASE_NAME}.o" -o "v2/components/runtime/data/test-elfs/${BASE_NAME}.so"
    fi
    
    if [[ " ${V0_FILES[@]} " =~ " ${BASE_NAME} " ]]; then
        $CC_V0 "v2/components/runtime/data/test-elfs/${BASE_NAME}.bc" -c -o "v2/components/runtime/data/test-elfs/${BASE_NAME}_sbpfv0.o"
        $LD_V0 "v2/components/runtime/data/test-elfs/${BASE_NAME}_sbpfv0.o" -o "v2/components/runtime/data/test-elfs/${BASE_NAME}_sbpfv0.so"
    fi
done

rm v2/components/runtime/data/test-elfs/*.o
rm v2/components/runtime/data/test-elfs/*.bc
...
python3 -c "
import struct, os
for fn in os.listdir('v2/components/runtime/data/test-elfs'):
    if not fn.endswith('.so'): continue
    path = f'v2/components/runtime/data/test-elfs/{fn}'
