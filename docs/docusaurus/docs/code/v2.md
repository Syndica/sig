...
v2 runs each service as an isolated process. Services typically cannot allocate memory or open files at runtime; any syscalls used must be whitelisted, as they are denied by default. 
Instead, they communicate through shared memory using fixed-size buffers created by the init process before any
...
The parent process needs to know how to initialize the region before spawning
