...
pub fn serviceMain(ro: ReadOnly, rw: ReadWrite) !noreturn {
    ...
    const Effects = struct {
        iter: *lib.net.Pair.PacketRing.Iterator(.writer),
        keypair: *const lib.gossip.KeyPair,

        const Self = @This();

        pub fn writePacket(self: Self) *Packet {
            while (true) return self.iter.next() orelse {
        ...
        pub fn identity(self: Self) Pubkey {
    ...
    var iter = rw.net_pair.send.get(.writer);
    ...
    var gossip = try Gossip(Effects).init(&fba, now, .{
        .effects = effects,
...
fn Gossip(comptime Effects: type) type {
    return struct {
        table: Table,
        peers: Peers,
        expired: Expired,
        ...
        fn identity(self: *const Self) Pubkey {
            return self.config.effects.identity();
        }
