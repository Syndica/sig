...
fn setRecvTimeOut(
...
) error{SetRecvTimeoutFailed}!void {
    const ret: std.os.linux.E = .init(std.os.linux.setsockopt(
        handle,
    ...
    if (ret != .SUCCESS) return error.SetRecvTimeoutFailed;
}

fn setSendTimeOut(
    handle: std.os.linux.socket_t,
    timeout: std.os.linux.timeval,
) error{SetSendTimeoutFailed}!void {
    const ret: std.os.linux.E = .init(std.os.linux.setsockopt(
        handle,
        std.os.linux.SOL.SOCKET,
    ...
    if (ret != .SUCCESS) return error.SetSendTimeoutFailed;
}
