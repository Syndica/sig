...
const Topology = struct {
    shred_stream: ServiceRegions(.from(services.shred_stream)),
...
pub fn main() !void {
    ...
    if (process_args.len < 2) return error.ConfigPathMissing;
    ...
    shred_recv_data.shred_version = config.shred_network.shred_version;
    ...
    var shred_stream_args_region: Region(shred_stream_api.Args) = try .simple();
    try shred_stream_args_region.ptr().init(shred_stream_args);
    ...
    try children.spawn(mode, .{
        .shred_stream = .{
            .ro = .{ .args = shred_stream_args_region.finish() },
            .rw = .{
