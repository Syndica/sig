...
pub fn serviceMain(ro: ReadOnly, rw: ReadWrite) !noreturn {
    ...
    { // wait until all pending services have registered their metrics
        const pending_services = &ro.init.pending_services;
    ...
    try collectMetrics(gpa, &metrics, .{
        .id_mem = ro.id_mem[0..ro.init.id_mem_end.load(.acquire)],
        .gauges = ro.gauges,
        .histogram_data = rw.histogram_data,
    });

    const listen_addr: std.net.Address = .initIp4(.{ 0, 0, 0, 0 }, ro.params.port);
    ...
    while (metric_id_r.bufferedLen() != 0) {
        ...
        gop.value_ptr.* = switch (detail.kind) {
            ...
            .histogram => .{ .histogram = histogram: {
                ...
                const element_count = api.Histogram.elementsFromBucketCount(bucket_count);
...
fn ordinalSuffix(value: anytype) []const u8 {
