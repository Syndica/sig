...
pub const Shred = union(ShredType) {
    ...
    pub fn fromPayload(data: []const u8) !Shred {
        const variant = layout.getShredVariant(data) orelse return error.InvalidShredVariant;
        return switch (variant.shred_type) {
            .code => .{ .code = try CodeShred.Generic.fromPayload(data) },
            .data => .{ .data = try DataShred.Generic.fromPayload(data) },
        };
    }
    ...
    pub fn verify(self: Shred, signer: Pubkey) !void {
        return switch (self) {
...
fn GenericShred(shred_type: ShredType) type {
    ...
    return struct {
        fn fromPayload(payload: []const u8) !Self {
            ...
            const valid_payload = payload[0..constants.payload_size];
        ...
        fn verify(self: Self, leader: Pubkey) !void {
...
pub const ShredFlags = packed struct(u8) {
    reference_tick_bits: u6,
...
test "fromPayload" {
    const shred = try Shred.fromPayload(&test_data_shred);
    _ = shred;
}
...
test "mainnet shreds look like agave" {
    const test_data = @import("shred/test_shreds.zig");
...
pub fn loadShredsFromFile(allocator: Allocator, path: []const u8) ![]const Shred {
    ...
    var shreds: std.ArrayList(Shred) = .{};
    errdefer shreds.deinit(allocator);

    while (try readChunk(allocator, &reader.interface)) |chunk| {
        defer allocator.free(chunk);

        const shred_payload_duped = try allocator.dupe(u8, chunk);
        errdefer allocator.free(shred_payload_duped);

        try shreds.append(allocator, try Shred.fromPayload(shred_payload_duped));
    }
...
fn readChunk(allocator: Allocator, reader: *std.io.Reader) !?[]const u8 {
    ...
    const num_bytes_read = try reader.readSliceShort(chunk);
...
test "merkle tree round trip" {
    ...
    var tree = try nodes.clone(allocator);
    defer tree.deinit(allocator);
    try makeMerkleTree(allocator, &tree);
    ...
    for (0..size) |index| {
        ...
        for (nodes.items, 0..) |node, k| {
            if (k == index) {
                ...
                const recalculated_root = try calculateMerkleRoot(k, node, owned_proof);
                try std.testing.expect(!root.eql(&recalculated_root));
            }
        }
    }
}
...
test makeMerkleTree {
    ...
    var nodes_list: std.ArrayList(Hash) = .{};
    defer nodes_list.deinit(std.testing.allocator);
    for (nodes) |n| try nodes_list.append(std.testing.allocator, n);
    try makeMerkleTree(std.testing.allocator, &nodes_list);
