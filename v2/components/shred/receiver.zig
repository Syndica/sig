...
pub const Receiver = struct {
    ...
    pub fn processPacket(
        ...
        // We now have a new shred that has passed validation, time to add it
        // to our in-progress fec set
        ...
        // writing out deshredded fec set
        {
            ...
            const finished: *DeshreddedFecSet = deshred_writer.next() orelse
                // If there's nowhere to write to, then this means that
                // services downstream haven't been keeping up for a while.
                // For now let's just exit if this happens, however this
                // might leave us vulnerable to denial of service.
                //
...
/// The two roots pinned for a completed FEC set.
...
pub const FecSetCtx = extern struct {
    ...
    merkle_root: Hash,
    // Merkle root of the previous FEC set (SIMD-0340). First-seen wins.
    chained_merkle_root: Hash,
