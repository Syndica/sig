...
pub fn executeNativeCpiInstruction(
    ...
    // account_metas memory to remain valid until the transaction completes. Cleanup happens in
...
fn processNextInstruction(
    ...
    // logging every call to it would generate a large number of logs that may not be useful for
    // end users.
    if (!is_precompile) {
...
pub fn prepareCpiInstructionInfo(
    ...
    for (callee.accounts) |account| {
        ...
        if (index_in_callee_ptr.* < deduped_account_metas.items.len) {
            ...
            // this avoids a bug caused by Parameter Reference Optimisation (PRO)
            deduped_account_metas.addOneAssumeCapacity().* = prev.*;
        } else {
