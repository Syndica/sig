...
pub const failing = struct {
    ...
    /// manner. By default each method is a simple failure or noop, and can be escalated to a panic
    /// which is enabled in safe and unsafe modes, or to an assertion which triggers checked illegal
    /// behaviour.
