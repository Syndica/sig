...
pub fn proofOfPossessionVerify(
    ...
    // Match firedancer's defensive minimum length to reject messages which do not
    // contain a bls pubkey (48 bytes).
    ...
    if (c.p1_uncompress(&pk, pubkey_compressed) != c.SUCCESS) {
        return error.PubkeyDecompressionFailed;
    }
...
// Cross-check against firedancer (alpenglow vector) to ensure our wire-up and DST
// usage matches.
