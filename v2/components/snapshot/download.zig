...
pub const DownloadConn = struct {
    ...
    /// the original peer's download was abandoned from overwritting the new
    /// peer it's now representing (i.e generations don't match).
    gen: u16,
    ...
    const DownloadRuntimeState = struct {
        ...
        from: Pubkey,
        /// Address used for the socket connection (must live long enough
        /// for io_uring use, hence stored here).
        net_addr: std.net.Address,
        ...
        bytes_written: u64,
        /// The number of bytes moved from the socket into the pipe that
        /// still need to be flushed to this peer's temp file.
        pipe_pending: u64,
        /// Monotonic per-download sequence for linked ops without a natural byte offset.
        op_seq: u16,
        /// Set once a winner is declared so signal that this download
        /// should be stopped and cleaned up.
...
pub const Downloader = struct {
    ...
    pub fn init(
        ...
        return .{
            ...
            .metrics = metrics,
            .logger = logger,
            .await_state = .init(AWAITING_LOG_INTERVAL_NS, AWAITING_WARN_AFTER_NS, now_ns),
        };
    }
    ...
    pub fn run(self: *Downloader) !DownloadResult {
        ...
        while (true) {
            ...
            // final result. The top-of-loop check
    ...
    fn handleProbeConnect(self: *Downloader, data: UserData, cqe: std.os.linux.io_uring_cqe) void {
        ...
        // if so we finish the probe with result .sq_full so the peer
        // remains .pending for retry later (back pressure).
    ...
    fn finishProbe(self: *Downloader, probe_index: u8, result: ProbeResult) void {
        ...
        const state = &active.state;
        if (self.dedupe_map.getPtr(state.addr)) |peer| {
            // NOTE: We guard the update here since gossip can update peer
            // in dedupe map while io_uring was completing this probe.
            // If it was updated underneath us, don't update things here
            // since another/newer probe was already issued to io_uring.
            if (peer.slot == state.slot and peer.hash.eql(&state.hash)) {
    ...
    fn handleDownloadFsync(self: *Downloader, data: UserData, cqe: std.os.linux.io_uring_cqe) void {
        ...
        // Since this is the final stage for fsyncing the downloaded file,
        // a failure here is likely not to resolve with retry/re-download
        // (local storage issue). So cancel all other running downloads
        // as well as with winner's.
    ...
    fn queueDownloadRecvHeadersWithTimeout(
        ...
        const conn = &self.download_conns[data.index];
        // NOTE: callers either just finished sending or are handling a
        // current-generation header recv CQE; queueing cannot retire
        // the slot.
        const active = conn.activePtr() orelse unreachable;
    ...
    /// `\r\n\r\n` and already consume some snapshot body bytes into
    /// recv_buf.
    fn queueDownloadWriteExtra(self: *Downloader, data: UserData, written: u16) QueueSqeError!void {
        ...
        // handling its current-generation CQE; queueing cannot retire
        // the slot.
        const active = conn.activePtr() orelse unreachable;
        const state = &active.state;
        const writing_extra = &active.phase.writing_extra;
    ...
    fn queueDownloadPollInWithTimeout(self: *Downloader, index: u8) QueueSqeError!void {
        const conn = &self.download_conns[index];
        // NOTE: callers reach this from a current-generation splice_in
        // CQE; queueing cannot retire the slot.
        const active = conn.activePtr() orelse unreachable;
        const state = &active.state;
    ...
    fn queueDownloadSpliceOut(self: *Downloader, index: u8) QueueSqeError!void {
        const conn = &self.download_conns[index];
        // NOTE: callers reach this while the current-generation download
        // has pipe bytes pending; queueing cannot retire the slot.
        const active = conn.activePtr() orelse unreachable;
        const state = &active.state;
    ...
    fn queueDownloadFsync(self: *Downloader, index: u8) QueueSqeError!void {
        const conn = &self.download_conns[index];
        // NOTE: callers reach this after a current-generation splice_out
        // CQE completes the download; queueing cannot retire the slot.
        const active = conn.activePtr() orelse unreachable;
        const state = &active.state;
