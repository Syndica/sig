...
/// Explicit numbers are added to ensure we don't mess up the order of
/// the fields and break bincode reading.
pub const Cluster = enum(u8) {
    ...
    pub fn getEntrypoints(self: Cluster) []const []const u8 {
        return switch (self) {
            .mainnet => &.{
