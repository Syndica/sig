...
/// Reading a `(incremental-)snapshot-{slot}-{hash}.tar.zst`
pub const SnapshotReader = struct {
    ...
    pub fn init(
        ...
        self.tar_iter = try .init(snapshot_dir, snapshot_path);
    ...
    pub fn deinit(self: *const SnapshotReader) void {
    ...
    pub const Account = struct {
        ...
        owner: Pubkey,
        lamports: u64,
        data: packed struct(u64){ executable: bool, len: u63 },
        rent_epoch: Epoch,
        slot: Slot,
    };

    pub fn next(self: *SnapshotReader) !?Account {
        // skip previous Account's data + padding
        const skip = self.account_data_len + self.account_data_padding;
        _ = try self.tar_iter.discardShort(skip);

        // read accounts/{slot}/{id} files containing AppendVecs (files with accounts in them)
        if (self.account_file_len == 0) {
        ...
        // read Account header within AppendVec
        ...
        try self.tar_iter.readSliceAll(std.mem.asBytes(&header));
        ...
        self.account_file_len -|= @sizeOf(@TypeOf(header)) + data_len_padded;
        self.account_data_padding = data_len_padded - header.data_len;
    ...
    // Read the account data from a previously returned `Account` from next()
    pub fn readSliceAll(self: *SnapshotReader, buf: []u8) !void {
...
const TarZstIter = struct {
    ...
    tar_file_size: usize = 0,
    tar_file_padding: usize = 0,
    ...
    pub fn init(snapshot_dir: std.fs.Dir, snapshot_path: []const u8) !Self {
        ...
        const decompressor: Decompressor = try .init(.{});
    ...
    pub fn next(self: *Self) !?TarFile {
        while (true) {
            _ = try self.read(null, self.tar_file_padding + self.tar_file_size);

            const n = try self.read(&self.tar_header, self.tar_header.len);
            if (n == 0) return null;
            ...
            self.tar_file_size = file_size;
            self.tar_file_padding = std.mem.alignForward(usize, file_size, 512) - file_size;
    ...
    pub fn readSliceAll(self: *Self, buf: []u8) !void {
        const take = @min(buf.len, self.tar_file_size);
        self.tar_file_size -= take;
        if (try self.read(buf.ptr, take) != buf.len) return error.EndOfStream;
    }
    ...
    pub fn discardShort(self: *Self, n: usize) !usize {
        const take = @min(n, self.tar_file_size);
        self.tar_file_size -= take;
        return try self.read(null, take);
    }

    fn read(self: *Self, maybe_buf: ?[*]u8, len: usize) !usize {
        var n: usize = 0;
        while (n < len) {
            if (self.reader.seek == self.reader.end) {
                @branchHint(.unlikely);

                const decompressed = &self.reader.buffer;
                const compressed = self.mapped[0..self.file_size][self.consumed..];
                ...
                _ = self.decompressor.decompressStream(&in, &out) catch return error.ReadFailed;

                self.consumed += in.pos;
                self.reader.seek = 0;
                self.reader.end = out.pos;
            }

            const readable = self.reader.buffer[self.reader.seek..self.reader.end];
            const take = @min(readable.len, len - n);
            if (maybe_buf) |buf| @memcpy(buf[n..][0..take], readable[0..take]);
            self.reader.seek += take;
            n += take;
        }
        return n;
    }
