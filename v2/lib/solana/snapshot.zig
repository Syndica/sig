    ...
    return struct {
        ...
        pub fn next(self: *Self) !?TarFile {
            while (true) {
                ...
                _ = try self.read(null, self.file_padding + self.file_size);

                const n = try self.read(&self.header, self.header.len);
                if (n == 0) return null;
        ...
        pub fn readSliceAll(self: *Self, buf: []u8) !void {
            ...
            if ((try self.read(buf.ptr, buf.len)) != buf.len) return error.EndOfStream;
        }

        pub fn discardAll(self: *Self, n: usize) !void {
            if (self.file_size < n) return error.EndOfStream;
            self.file_size -= n;
            if ((try self.read(null, n)) != n) return error.EndOfStream;
        }
