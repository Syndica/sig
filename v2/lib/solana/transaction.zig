...
pub const VersionedTransaction = struct {
    ...
    /// Returns `true` iff the transaction satisfies the parse-level
    /// structural invariants the protocol places on a
    /// `VersionedTransaction`. The MTU bound is checked separately
    /// (`serializedSize` ≤ `MAX_BYTES`), as is the duplicate-account-key
    /// check (`validateAccountLocks`).
    ///
    /// Mirrors agave's `transaction-view::sanitize` exactly: the same
    /// invariants in the same order with the same accept/reject decision,
    /// so a conformance harness comparing per-stage error codes sees the
    /// same result here as agave produces at its `SanitizeMessageError`
    /// boundary. Checks that agave defers to later stages (total
    /// addressable account count, instruction-index reachability through
    /// ALT-loaded keys) are *not* enforced here — they're caught
    /// downstream at account-locks / message-resolution time and pulling
    /// them forward would change which stage rejects.
    ///
    ...
    pub fn sanitize(self: VersionedTransaction) bool {
        ...
        // (10) + (11): each ALT must contribute at least one index, and
        // neither index list may exceed the remaining static-keys headroom.
        // Subtraction `MAX_ACCOUNT_ADDRESSES - acct_cnt` is safe since
        // check 4 bounded `acct_cnt`.
...
//
// Each case maps to one of the 11 numbered invariants in the doc-comment
