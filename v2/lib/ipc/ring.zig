    ...
    return extern struct {
        ...
        pub fn Iterator(comptime side: Side) type {
            return struct {
                ...
                // Using the increments done via `next()` since either
                // 1) the .get() creating this Iterator or 2) the last
                // markUsed() call, update the position of this side on
                // the ring buffer, making any changes to the memory
