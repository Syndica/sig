...
pub const MerkleNode = extern struct {
    ...
    id: FecSetId,
    /// `shred.slot - parent_slot`. All data shreds in a FEC set share this
    /// (merkle-hashed DataHeader field), so the FEC set can quote a single
    /// value; the harness reads it back to detect per-slot parent-slot
    /// divergence in `deriveBlockParseResult`.
    parent_offset: u16,
    data_complete: bool,
    slot_complete: bool,
...
pub const MerkleForest = struct {
    ...
    /// Reset to the post-init state without freeing any heap memory. The
    /// backing pool buffer is clobbered and the map/orphan_map keep their
    /// current capacities. Intended for callers (e.g. the conformance
    /// harness) that reuse a single forest across many independent inputs.
...
fn attachParent(
    ...
    const parent = forest.map.getAdapted(&node.chained_merkle_root, map_ctx) orelse {
        ...
        if (orphan_map_result.found_existing) {
            ...
            while (orphan_sibling_tail) |tail_node| {
                ...
                // Adjacent orphans sharing a FecSetId are equivocating
                // variants (different merkle_root, so distinct entries in
                // `forest.map`); keep both — consensus (Phase N) or the
                // harness (`deriveBlockParseResult`) sorts them out.
    ...
    // insert node into tree of parent
    {
        ...
        while (true) {
            // A child with the same FecSetId is an equivocating variant
            // (different merkle_root by construction). Both survive as
            // siblings under this parent; consensus (Phase N) or the
            // harness's `deriveBlockParseResult` folds the divergence.
