...
pub const ExecutionRegistry = extern struct {
    ...
    pub const TxSegmentNode = extern struct {
        keys: [tx_per_segment]KeySlice,
        blocks: [tx_per_segment]BlockRef.Optional,
        next: Id.Optional,
        ...
        pub fn init(self: *TxSegmentNode) void {
            @memset(&self.keys, tx_hash_id_sentinel);
        ...
        fn getOrFindPlace(
            segment: *const TxSegmentNode,
            block_ref: BlockRef,
            lookup_key: *const KeySlice,
        ) LookupResult {
            std.debug.assert(!std.mem.eql(u8, lookup_key, &tx_hash_id_sentinel));
            for (&segment.keys, &segment.blocks, 0..) |*tx_hash_slice, tx_hash_block_opt, i| {
                const tx_hash_block = tx_hash_block_opt.opt() orelse {
                    std.debug.assert(std.mem.eql(u8, tx_hash_slice, &tx_hash_id_sentinel));
                    std.debug.assert(segment.next == .null);
                    return .{ .vacant = i };
                };
                if (block_ref == tx_hash_block and std.mem.eql(u8, tx_hash_slice, lookup_key)) {
                    return .{ .match = i };
                }
            }
    ...
    pub fn containsTransaction(
        ...
        /// The block (of the implied fork) in which the transaction was executed.
        block_ref: BlockRef,
        /// The transaction key.
        ...
        while (current.opt()) |segment_id| {
            ...
            switch (segment.getOrFindPlace(block_ref, lookup_key)) {
                .match => return true,
    ...
    pub fn insert(
        ...
        /// Block that's associated with the `recent_blockhash` of interest, with the implied ancestors.
        recent_block_ref: BlockRef,
        /// The block on which the transaction was executed.
        block_ref: BlockRef,
        ...
        const first_segment_id = entry.transactions.opt() orelse {
            entry.transactions = .init(try exec_registry.makeNewSegment(block_ref, tx_key));
            return;
        };
        const first_segment = first_segment_id.ptr(&exec_registry.tx_segment_pool);
        switch (first_segment.getOrFindPlace(block_ref, tx_key)) {
            .match => return,
            .vacant => |vacancy| {
                first_segment.keys[vacancy] = tx_key.*;
                first_segment.blocks[vacancy] = .init(block_ref);
                return;
            },
            .full_no_match => {},
        }
        ...
        while (true) {
            ...
            const current_child_id = current_parent.next.opt() orelse {
                current_parent.next = .init(try exec_registry.makeNewSegment(block_ref, tx_key));
                return;
            };
            defer current_parent_id = current_child_id;
            const current_child = current_child_id.ptr(&exec_registry.tx_segment_pool);
            switch (current_child.getOrFindPlace(block_ref, tx_key)) {
                .match => return,
                .vacant => |vacancy| {
                    current_child.keys[vacancy] = tx_key.*;
                    current_child.blocks[vacancy] = .init(block_ref);
                    return;
                },
                .full_no_match => {},
            }
    ...
    fn makeNewSegment(
        ...
        tx_entry.init();
        tx_entry.keys[0] = lookup_key.*;
        tx_entry.blocks[0] = .init(block_ref);
        return tx_entry_id;
    }
