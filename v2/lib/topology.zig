    ...
    return struct {
        ...
        pub fn spawnSandboxed(map: *const ServiceMap) !Sandboxed {
            ...
            for (metas.items) |*exit_meta| {
                ...
                exit_meta.runner = try entry.runner.mmapStaticSize(lib.runner.Region, null);
            }
        ...
        pub fn spawnNoSandbox(
            ...
            for (map.entries.keys(), map.entries.values()) |k, v| {
                state.runners.putNoClobber(k, try v.runner.mmapStaticSize(lib.runner.Region, null));
            }
        }
