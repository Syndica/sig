...
pub const Schema = struct {
    ...
    pub const Service = struct {
        ...
        /// Per-service syscall allowlist for seccomp BPF filter generation.
        syscalls: []const Syscall = &.{},

        pub const Syscall = struct {
            syscall: @Type(.enum_literal),
        };
...
pub fn Unbound(comptime schema: Schema) type {
    return struct {
        ...
        pub fn getServiceSyscalls(service: ServiceId) []const lib.linux.bpf.SyscallConfig {
            return switch (service) {
                inline else => |s| comptime blk: {
                    const service_schema = schema.services[@intFromEnum(s)];
                    var configs: []const lib.linux.bpf.SyscallConfig = &.{};
                    for (service_schema.syscalls) |sc| {
                        // Validate syscall name exists in target SYS enum (compile error if not)
                        _ = @field(std.os.linux.SYS, @tagName(sc.syscall));
...
pub fn Bind(
    ...
    return struct {
        ...
        fn spawnService(
            ...
            {
                const syscall_configs = comptime unbound.getServiceSyscalls(service);
                const bpf_filters = lib.linux.bpf.serviceFilter(syscall_configs, params.stderr.handle);
