    ...
    return struct {
        ...
        pub fn processPacket(
            ...
            const msg = bincode.read(&msg_fba, &msg_reader, GossipMessage) catch |e| {
                logger.err().logf("invalid msg from ({f}, size={}): {}", .{ packet.addr, packet.size, e });
                return;
            };

            self.processMessage(.from(logger), now, packet.addr, msg) catch |e| {
                logger.err().logf("failed to process msg ({f}, {s}) {}", .{ packet.addr, @tagName(msg), e });
                return;
            };
        }
            ...
            switch (msg) {
                ...
                .ping_message => |ping| {
                    ping.signature.verify(&ping.from, &ping.token) catch {
                        logger.err().logf("invalid Ping signature from {f}:{f}", .{ ping.from, addr });
                        return;
                    };
                ...
                .pong_message => |pong| {
                    ...
                    if (!self.ping_window.contains(&pong.hash)) {
                        logger.err().logf("invalid Pong hash from {f}:{f}", .{ pong.from, addr });
                        return;
                    }

                    pong.signature.verify(&pong.from, &pong.hash.data) catch {
                        logger.err().logf("invalid Pong signature from {f}:{f}", .{ pong.from, addr });
                        return;
                    };

                    const peer = self.peers.getPtr(pong.from) orelse {
                        logger.err().logf("pong from untracked peer {f}:{f}", .{ pong.from, addr });
                        return;
                    };
