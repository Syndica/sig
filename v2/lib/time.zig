...
pub const Instant = struct {
    ...
    /// This assumes that the `earlier` Instant represents a moment in time
    /// before or equal to `self`.
    /// This also assumes that the time that has passed between both Instants
    /// fits inside a u64 (~585 yrs).
