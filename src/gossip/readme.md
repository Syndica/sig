...
Push messages are generated by finding `CrdsValues` which have been inserted in the crds table after the last push message has been sent (ie, new crds values). To do this, we track a `push_cursor: usize` variable and the getter function `crds_table.get_entries_with_cursor(...)` to get new crds values which have been inserted with a cursor greater than `push_cursor`.
...
An important note is that all messages sent through gossip should be less than or equal to a [maximum tramission unit (MTU)](https://en.wikipedia.org/wiki/Maximum_transmission_unit) of 1280 bytes (which is refered to as a `Packet` struct throughout the codebase).
...
When building the active set, the number of leading zeros in the local node's stake weight decides which entry to sample the nodes from. For example, if the local node has a large stake, it will have a small number of leading zeros, and so its index will be large. The large index will correspond to an entry which has a higher probability of selecting a high-staked node to be included in the active set. 

**This all means, high stake nodes are more likely to send push messages to other high stake nodes, while low stake nodes send push messages to random nodes**

### Recieving Push Messages 

When recieving a new `PushMessage`, the values are inserted into the CrdsTable. While inserting these values, duplicate values are tracked, and nodes which send a large number of duplicates are then sent prune messages as a way to say, 'stop sending me this duplicate data'.
