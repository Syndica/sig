...
The `PingCache` defines a few important variables including the amount of time a recieved pong is valid for - ie, when the node should send a new ping message - which is set to 1280 seconds, and the rate limit for how often to send Ping messages (do we dont spam nodes) which is set to only send Ping messages every 20 seconds. 
