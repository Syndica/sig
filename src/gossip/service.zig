...
test "handling prune messages" {
    ...
    for (0..10) |_| {
        var rand_keypair = try KeyPair.create(null);
        const value =
            try SignedGossipData.randomWithIndex(prng.random(), &rand_keypair, 0); // contact info
        _ = try lg.mut().insert(value, getWallclockMs());
        try peers.append(
...
test "init, exit, and deinit" {
    ...
    var contact_info =
        try LegacyContactInfo.initRandom(prng.random()).toContactInfo(std.testing.allocator);
