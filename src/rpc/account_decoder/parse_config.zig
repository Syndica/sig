...
fn parseStakeConfig(allocator: Allocator, data: []const u8) ParseError!?ConfigAccountType {
    ...
    return ConfigAccountType{
        .stake_config = UiStakeConfig{
            .warmup_cooldown_rate = stake_config.warmup_cooldown_rate,
            .slash_penalty = stake_config.slash_penalty,
        },
    };
}
...
fn parseValidatorInfo(allocator: Allocator, data: []const u8) ParseError!?ConfigAccountType {
    ...
    // Build UI keys array
    const ui_keys = allocator.alloc(UiConfigKey, config_keys.keys.len) catch
        return ParseError.OutOfMemory;
    for (config_keys.keys, 0..) |key, i| {
        ui_keys[i] = UiConfigKey{
            .pubkey = key.pubkey.base58String(),
            .signer = key.is_signer,
        };
    }
    // Copy the info string (we need to own it since we're freeing validator_info)
    const info_copy = allocator.dupe(u8, validator_info.info) catch
        return ParseError.OutOfMemory;
    return ConfigAccountType{
        .validator_info = UiConfig{
            .keys = ui_keys,
            .config_data = info_copy,
        },
    };
}
...
pub const UiStakeConfig = struct {
    warmup_cooldown_rate: f64,
    slash_penalty: u8,
    pub fn jsonStringify(self: UiStakeConfig, jw: anytype) @TypeOf(jw.*).Error!void {
        try jw.beginObject();
        try jw.objectField("warmupCooldownRate");
        try jw.write(self.warmup_cooldown_rate);
        try jw.objectField("slashPenalty");
        try jw.write(self.slash_penalty);
        try jw.endObject();
    }
};
...
pub const UiConfigKey = struct {
    pubkey: Pubkey.Base58String,
    signer: bool,
    pub fn jsonStringify(self: UiConfigKey, jw: anytype) @TypeOf(jw.*).Error!void {
        try jw.beginObject();
        try jw.objectField("pubkey");
        try jw.write(self.pubkey.slice());
...
pub const ConfigAccountType = union(enum) {
    ...
    pub fn jsonStringify(self: ConfigAccountType, jw: anytype) @TypeOf(jw.*).Error!void {
        ...
        switch (self) {
            .stake_config => |v| {
                try jw.write("stakeConfig");
                try jw.objectField("info");
                try v.jsonStringify(jw);
            },
            .validator_info => |v| {
                try jw.write("validatorInfo");
                try jw.objectField("info");
                try v.jsonStringify(jw);
            },
        }
...
test "rpc.account_decoder.parse_config: parse config accounts" {
    ...
    // Test StakeConfig
    {
        ...
        const result = try parseConfig(allocator, ids.STAKE_CONFIG_PROGRAM_ID, stream.reader(), @intCast(full_data.len));
        try std.testing.expect(result != null);
        try std.testing.expect(result.? == .stake_config);

        const ui_stake = result.?.stake_config;
        try std.testing.expectEqual(@as(f64, 0.25), ui_stake.warmup_cooldown_rate);
        try std.testing.expectEqual(@as(u8, 12), ui_stake.slash_penalty);
    }
    ...
    // Test unknown config type (first key is not ValidatorInfo ID)
    {
        ...
        const result = try parseConfig(allocator, random_pubkey, stream.reader(), @intCast(keys_data.len));
        try std.testing.expect(result == null);
    }
    ...
    // Test invalid data
    {
        ...
        const result = parseConfig(allocator, ids.STAKE_CONFIG_PROGRAM_ID, stream.reader(), bad_data.len);
