...
test "blockSubscribe finalized: subscribe, notify, unsubscribe" {
    ...
    // The test ledger has no block data, so block is null with an error.
...
test "blockSubscribe without ledger: no notification sent" {
    const allocator = std.testing.allocator;

    // Start without ledger — blockSubscribe should silently skip.
    ...
    // Trigger finalized events — but no ledger means no block notification.
    server.injectEvent(.{ .slot_frozen = .{
        .slot = 50,
        .parent = 0,
        .root = 0,
    ...
    // Give the server some time to process; should NOT get a second message.
