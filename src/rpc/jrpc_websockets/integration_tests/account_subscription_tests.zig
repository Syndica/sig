...
test "accountSubscribe does not replay already-visible finalized account on later rooted slot" {
    const allocator = std.testing.allocator;
    ...
    const updated_account_shared = try putAccountAtSlot(
        server,
        5,
        pubkey,
        owner_pk,
    ...
    defer updated_account_shared.deinit(allocator);
    ...
    try std.testing.expectEqualStrings("accountNotification", notification.method);
    try std.testing.expectEqual(55_000, notification.params.result.value.lamports);
    try std.testing.expectEqual(5, notification.params.result.context.slot);
