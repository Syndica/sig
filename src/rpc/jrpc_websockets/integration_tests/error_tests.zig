...
test "duplicate subscribe returns error" {
    ...
    const parsed_dup = try std.json.parseFromSlice(
        JsonRpcErrorResponse,
        allocator,
        handler.received.items[1],
        .{ .ignore_unknown_fields = true },
    );
    defer parsed_dup.deinit();
    const dup = parsed_dup.value;

    try std.testing.expectEqual(2, dup.id orelse return error.TestUnexpectedResult);
    try std.testing.expectEqual(-32602, dup.@"error".code);
    try std.testing.expect(
        std.mem.indexOf(u8, dup.@"error".message, "duplicate subscription") != null,
    );
