...
/// Max tracked slots retained, used to avoid unbounded memory growth
/// if we receive unexpected event patterns. Under normal operation
/// eviction should occur when slots are rooted.
...
pub fn onSlotConfirmed(
    ...
    const slot_state = self.getOrPutSlot(
        allocator,
        slot,
    ...
    var transition: Transition = .{
        .notify_commitments = .{ .confirmed = true },
    };
...
test "confirmed before frozen caches then freeze populates" {
    const allocator = std.testing.allocator;
    var slot_tracker = try sig.replay.trackers.SlotTracker
        .initEmpty(allocator, 0);
    defer slot_tracker.deinit(allocator);

    var commitments = sig.replay.trackers.CommitmentTracker.init(allocator, 0);
    defer commitments.deinit(allocator);

    try testAddTrackedSlot(
        allocator,
        &slot_tracker,
        10,
        9,
    ...
    // Confirm arrives before freeze: creates an empty
    // CachedSlot with confirmed=true.
    const confirm_t = try state.onSlotConfirmed(
        allocator,
        10,
    );
    try std.testing.expect(
        confirm_t.publishable_slot == null,
    );
    try std.testing.expect(
        confirm_t.notify_commitments.confirmed,
    );
    ...
    const frozen_t = try testOnSlotFrozen(
        &state,
        allocator,
        10,
        9,
    ...
    try std.testing.expect(
        frozen_t.notify_commitments.processed,
    );
    try std.testing.expect(
        frozen_t.notify_commitments.confirmed,
    );
    try std.testing.expect(
        !frozen_t.notify_commitments.finalized,
    );
...
test "capacity eviction removes minimum slot" {
    ...
    try std.testing.expect(state.cached_slots.getPtr(1) == null);
    try std.testing.expect(
        state.cached_slots.getPtr(slot_limit + 1) != null,
    );
}
...
test "gossip confirmations create unfrozen cache entries" {
    ...
    try std.testing.expect(
        state.cached_slots.getPtr(1) != null,
    );
    try std.testing.expect(
        state.cached_slots.getPtr(2) != null,
    );
...
test "rooted before frozen caches then freeze populates" {
    ...
    // Root arrives before freeze: creates an empty
    // CachedSlot with confirmed+rooted flags.
    const root_t = try state.onSlotRooted(allocator, 20);
    try std.testing.expect(root_t.publishable_slot == null);
    try std.testing.expect(
        root_t.notify_commitments.confirmed,
    );
    try std.testing.expect(
        root_t.notify_commitments.finalized,
    );
    try std.testing.expectEqual(
        @as(?Slot, 20),
        root_t.evict_through,
    );
