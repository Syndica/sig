...
pub fn serializeAccountNotification(
    ...
    const encoded_data: account_codec.AccountData = if (encoding == .jsonParsed) blk: {
        const ctx = read_ctx orelse return error.MissingReadContext;
        const tracker = @constCast(ctx.slot_tracker);
        const slot_ref = tracker.get(data.slot) orelse
...
pub fn serializeNotification(
    ...
    return switch (event_data) {
        ...
        .account => |d| switch (sub_method) {
            .account => serializeAccountNotification(
                allocator,
                sub_id,
                d,
                encoding,
            ...
            .program => serializeProgramNotification(
                allocator,
                sub_id,
                d,
                encoding,
