...
pub const GetTokenAccountsByOwner = struct {
    ...
    pub const TokenAccountsFilter = union(enum) {
        ...
        pub fn resolve(
            ...
            switch (self) {
                .mint => |mint_pubkey| {
                    // Fetch the mint account to determine its owning token program.
        ...
        pub fn jsonParseFromValue(
            ...
            // Agave requires exactly one key — reject if both "mint" and "programId" are present.
            if (obj.count() != 1) return error.UnexpectedToken;
            if (obj.get("mint")) |val|
                return .{ .mint = try std.json.innerParseFromValue(Pubkey, allocator, val, options) };
            if (obj.get("programId")) |val|
                return .{ .programId = try std.json.innerParseFromValue(Pubkey, allocator, val, options) };
            return error.UnexpectedToken;
        }
    };

    /// Uses `RpcAccountInfoConfig` (not `RpcProgramAccountsConfig`) — no user-supplied filters,
