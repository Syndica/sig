...
pub const MockTransferService = struct {
    ...
    const bank_kp: KeyPair = .{
        ...
        .secret_key = .{ .bytes = .{
            208, 26,  255, 64,  164, 52,  99,  120, 92, 227, 25,  240, 222,
            245, 70,  77,  171, 89,  129, 64,  110, 73, 159, 230, 38,  212,
            150, 202, 57,  157, 151, 175, 239, 10,  4,  236, 219, 237, 69,
            197, 199, 60,  117, 184, 223, 215, 132, 73, 93,  248, 200, 254,
    ...
    pub fn run(self: *MockTransferService) !void {
        ...
        self.logger.debugf("bank key: {}", .{bank_pubkey});
        ...
        if (bank_balance.value < 1) {
            const airdrop_response = try rpc_client.requestAirDrop(
                self.allocator,
                bank_pubkey,
                1e9, // 1 SOL
                .{},
            );
            defer airdrop_response.deinit();
        }
        ...
        self.logger.debugf("account a: {}", .{account_a_pubkey});
        self.logger.debugf("account b: {}", .{account_b_pubkey});
