...
pub fn handleTransactions(
    ...
    try txn_pool.ensureTotalCapacity(gpa, config.max_pooled);

    var drop_list = std.ArrayList(Signature).empty;
    defer drop_list.deinit(gpa);
    try drop_list.ensureTotalCapacity(gpa, config.max_pooled);
    ...
    while (!exit.shouldExit()) {
        ...
        const ctx = loadProcessContext(
            gpa,
            account_store,
            slot_tracker,
            &leader_info,
        ) catch |err| {
            logger.warn().logf("Failed to load context: err={any}", .{err});
        ...
        try processTransactions(
            ...
            status_cache,
            ctx.root_ancestors,
            ctx.root_block_height,
            ctx.working_ancestors,
            ctx.account_reader,
            ctx.leader_addresses,
        );
    }
}
...
const LeaderInfo = struct {
    ...
    pub fn getLeaderAddresses(self: *LeaderInfo, slot: Slot, buf: []?sig.net.SocketAddr) ![]const ?sig.net.SocketAddr {
        ...
        return self.leader_addresses_buf;
    }
    ...
    fn getLeaderAddress(self: *LeaderInfo, slot: u64) !?sig.net.SocketAddr {
        ...
        if (self.leader_addresses.count() >= self.leader_addresses.capacity() - 1) {
            ...
            while (self.leader_addresses.count() >=
                self.leader_addresses.capacity() - 1) : (threshold /= 2)
            {
