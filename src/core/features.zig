...
/// Represents the set of currently enabled feature flats.
///
...
pub const Set = struct {
    ...
    pub fn setSlotId(self: *Set, id: u64, slot: Slot) !void {
        for (&self.array.values, 0..) |*destination, i| {
            const feature: Feature = @enumFromInt(i);
            const info = map.get(feature);
            const feature_id: u64 = @bitCast(info.data[0..8].*);
            if (feature_id != id) continue;
            destination.* = slot;
            return;
        }
        return error.InvalidPubkey;
    ...
    const FullInflationFeatures = struct {
        ...
        pub fn enabled(self: FullInflationFeatures, set: Set, slot: Slot) bool {
            if (self.mainnet and !set.active(.full_inflation_mainnet_enable, slot)) return false;
            if (self.devnet_and_testnet and
                !set.active(.full_inflation_devnet_and_testnet, slot)) return false;
            return true;
        }
    };
    ...
    const Iterator = struct {
        ...
        pub fn next(self: *Iterator) ?Feature {
            while (true) : (self.index += 1) {
                ...
                if (self.set.active(feature, self.slot)) return feature;
            }
        }
    };
};
