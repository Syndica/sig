...
pub const Stakes = struct {
    ...
    pub fn deinit(self: Stakes, allocator: Allocator) void {
        var votes = self.vote_accounts.vote_accounts;
        votes.deinit(allocator);
        var nodes = self.vote_accounts.staked_nodes;
...
pub const VoteAccounts = struct {
    ...
    pub fn addStake(
        ...
        entry.stake += delta;
        const vote_account = try entry.account.clone(allocator);
        try self.addNodeStake(allocator, vote_account.state.node_pubkey, delta);
    }
...
test "stakes basic" {
    ...
    //     const accs = try createStakedNodeAccounts(allocator, prng.random(), 10);
