...
pub const StatusCache = struct {
    ...
    pub fn getStatus(
        ...
        return findVisibleFork(&state.get().roots, ancestors, stored_forks);
    }

    pub fn getOwnedStatusAnyBlockhash(
        self: *StatusCache,
        allocator: std.mem.Allocator,
        key: []const u8,
        ancestors: *const Ancestors,
    ) error{OutOfMemory}!?Fork {
        const zone = tracy.Zone.init(@src(), .{ .name = "StatusCache.getOwnedStatusAnyBlockhash" });
        defer zone.deinit();

        var state = self.state.read();
        defer state.unlock();

        for (state.get().cache.values()) |highest_fork| {
            const max_key_index = key.len -| (CACHED_KEY_SIZE + 1);
            const index = @min(highest_fork.index, max_key_index);
            const lookup_key: [CACHED_KEY_SIZE]u8 = key[index..][0..CACHED_KEY_SIZE].*;
            const stored_forks: ForkStatus = highest_fork.key_map.get(lookup_key) orelse continue;
            if (findVisibleFork(&state.get().roots, ancestors, stored_forks)) |fork| {
                return .{
                    .slot = fork.slot,
                    .maybe_err = if (fork.maybe_err) |err| try err.clone(allocator) else null,
                };
            }
        }
        return null;
...
test "status cache finds status across any cached blockhash" {
    const allocator = std.testing.allocator;
    var prng = std.Random.DefaultPrng.init(std.testing.random_seed);
    const random = prng.random();

    const signature = sig.core.Signature.ZEROES;
    const blockhash_a = Hash.ZEROES;
    const blockhash_b = Hash.init("blockhash_b");

    var ancestors = try Ancestors.initWithSlots(allocator, &.{6});
    defer ancestors.deinit(allocator);

    var status_cache: StatusCache = .DEFAULT;
    defer status_cache.deinit(allocator);

    try status_cache.insert(allocator, random, &blockhash_a, &signature.toBytes(), 5, null);
    try status_cache.insert(allocator, random, &blockhash_b, &signature.toBytes(), 6, null);

    try std.testing.expectEqual(
        Fork{ .slot = 6 },
        try status_cache.getOwnedStatusAnyBlockhash(allocator, &signature.toBytes(), &ancestors),
    );
}
...
test "status cache retains transaction errors for any-blockhash lookup" {
    ...
    const maybe_got = try status_cache.getOwnedStatusAnyBlockhash(
        allocator,
        &signature.toBytes(),
        &ancestors,
    );
