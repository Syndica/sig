...
pub fn requestIpEcho(
    ...
    const encoded = sig.bincode.writeToSlice(&write_buf, message, .{}) catch
        return error.EncodingError;
    try conn.writeAll(encoded);
    try conn.writeAll("\n");

    var buff: [32]u8 = undefined;
    var read_buf: [64]u8 = undefined;
    var reader = conn.reader(&read_buf);
    const bytes_read = try reader.interface().readSliceShort(&buff);
    if (bytes_read < HEADER_LENGTH + @sizeOf(IpEchoResponse)) {
        return error.InsufficientData;
    }
    var fbs = std.io.fixedBufferStream(buff[HEADER_LENGTH..bytes_read]);
