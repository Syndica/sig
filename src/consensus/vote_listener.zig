...
test ClusterInfoVoteListener {
    ...
    const node_keypair = try sig.identity.KeyPair.create(.{1} ** 32);
    const vote_keypair1 = try sig.identity.KeyPair.create(.{2} ** 32);
    const vote_keypair2 = try sig.identity.KeyPair.create(.{3} ** 32);
...

    {
        ...
        for (0..2) |i| {
            ...
            const transaction = try newTowerSyncTransaction(gossip_table.gossip_data_allocator, .{
