...
test "trackNewVotesAndNotifyConfirmations records confirmed slots" {
    ...
    try std.testing.expectEqual(@as(usize, 1), new_optimistic_confirmed_slots.items.len);
    try std.testing.expectEqual(@as(Slot, 1), new_optimistic_confirmed_slots.items[0].slot);
    try std.testing.expectEqual(
        @as(Slot, 1),
        slot_data_provider.slot_tracker.getSlotForCommitment(.confirmed),
    );
}
