...
const LatestVotes = std.AutoArrayHashMapUnmanaged(Pubkey, struct { slot: Slot, hashes: std.ArrayListUnmanaged(Hash) });
...
pub const LatestValidatorVotesForFrozenBanks = struct {
    ...
    pub fn deinit(self: *LatestValidatorVotesForFrozenBanks, allocator: std.mem.Allocator) void {
    ...
    pub fn maxGossipFrozenVotes(self: *const LatestValidatorVotesForFrozenBanks) *const LatestVotes {
...
pub fn latestVote(
    lastest_votes: *const LatestValidatorVotesForFrozenBanks,
    pubkey: Pubkey,
    is_replay_vote: bool,
...
fn runFrozenBanksCheckAddVoteIsReplayTest(allocator: std.mem.Allocator, is_replay_vote: bool) !void {
    ...
    var latest_validator_votes_for_frozen_banks: LatestValidatorVotesForFrozenBanks = .empty;
    defer latest_validator_votes_for_frozen_banks.deinit(allocator);
    ...
    // Case 1: Non-frozen banks shouldn't be added
    {
        ...
        const result1 = try latest_validator_votes_for_frozen_banks.checkAddVote(
            allocator,
            vote_pubkey,
            vote_slot,
            frozen_hash,
        ...
        try testing.expectEqual(0, latest_validator_votes_for_frozen_banks.max_replay_frozen_votes.count());
        try testing.expectEqual(0, latest_validator_votes_for_frozen_banks.max_gossip_frozen_votes.count());
        try testing.expectEqual(0, latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.count());
    }
    ...
    // shouldn't update state
    {
        ...
        for (0..num_repeated_iterations) |i| {
            ...
            const result = try latest_validator_votes_for_frozen_banks.checkAddVote(
                allocator,
                vote_pubkey,
                vote_slot,
                frozen_hash,
            ...
            if (is_replay_vote) {
                const dirty_entry = latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.get(vote_pubkey);
                ...
                try testing.expect(!latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.contains(vote_pubkey));
            }
        }
    }
    ...
    const all_frozen_hashes = [2]Hash{ frozen_hash, duplicate_frozen_hash };
    {
        ...
        if (is_replay_vote) {
            ...
            try testing.expect(
                !latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.contains(vote_pubkey),
            );
        }
    }
    ...
    // Case 7: Adding a vote for a new pubkey should also update the state
    {
        ...
        if (is_replay_vote) {
            const dirty_entry = latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.get(new_vote_pubkey);
            try testing.expect(dirty_entry != null);
            try testing.expectEqual(dirty_entry.?.slot, vote_slot);
            try testing.expectEqual(dirty_entry.?.hashes.items[0], new_frozen_hash2);
        } else {
            try testing.expect(
                !latest_validator_votes_for_frozen_banks.fork_choice_dirty_set.contains(new_vote_pubkey),
            );
        }
    }
}
...
fn setupDirtySet(
    allocator: std.mem.Allocator,
    random: std.Random,
    lvvfb: *LatestValidatorVotesForFrozenBanks,
    num_validators: u64,
    is_replay: bool,
...
fn runFrozenBanksTakeVotesDirtySet(allocator: std.mem.Allocator, is_replay: bool) !void {
    ...
    // Test case 1: Taking all the dirty votes >= 0 will return everything
    {
        ...
        var result = try latest_validator_votes_for_frozen_banks.takeVotesDirtySet(allocator, 0);
    ...
    // Test case 2: Taking all the dirty votes >= num_validators - 1 will only return the last vote
    {
        ...
        var result =
            try latest_validator_votes_for_frozen_banks.takeVotesDirtySet(allocator, 0);
        defer result.deinit(allocator);
        try std.testing.expect(result.items.len == 0);
    }
}
...
fn sortPubkeySlotAndHash(slice: []struct { Pubkey, SlotAndHash }) void {
    ...
    std.mem.sort(
        ...
        {},
        struct {
            fn lessThan(_: void, a: struct { Pubkey, SlotAndHash }, b: struct { Pubkey, SlotAndHash }) bool {
...
test "latest_validator_votes_for_frozen_banks_check_add_vote_is_replay" {
    try runFrozenBanksCheckAddVoteIsReplayTest(std.testing.allocator, true);
}

test "latest_validator_votes_for_frozen_banks_check_add_vote_is_not_replay" {
    try runFrozenBanksCheckAddVoteIsReplayTest(std.testing.allocator, false);
}

test "latest_validator_votes_for_frozen_banks_take_votes_dirty_set_is_replay" {
    try runFrozenBanksTakeVotesDirtySet(std.testing.allocator, true);
}

test "latest_validator_votes_for_frozen_banks_take_votes_dirty_set_is_not_replay" {
