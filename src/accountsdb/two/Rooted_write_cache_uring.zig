...
const Disk = struct {
    ...
    const max_write_buf = 64*1024*1024;
    ...
    fn open(file_path: [:0]const u8) !Disk {
        ...
        return .{
            ...
            .writes = .{
                .mapped = write_map[0..max_write_buf],
                .ring = try .init(@min(4096, max_write_queue), 0),
            },
        };
    }
    ...
    fn getWriteBuffer(self: *const Disk) []u8 {
        return self.writes.mapped[@sizeOf(WriteMap)..];
    }
    ...
    fn queueWrite(self: *Disk, op: WriteOp, aligned: u32) !WriteBufOffset {
        while (true) {
            ...
            if (new_size > max_write_buf or new_queued > max_write_queue) {
                @branchHint(.unlikely);
    ...
    fn flushWrites(self: *Disk) !void {
        ...
        if (self.writes.queued == 0) return;

        const ring = &self.writes.ring;
        for (&self.getWriteMap().array, 0..) |entry, idx| {
            if (@as(u64, @bitCast(entry.op)) == 0) continue;
            try self.submitEntry(idx, 0);
        }

        {
            const flush_zone = tracy.Zone.init(@src(), .{ .name = "Disk.ringWait" });
            defer flush_zone.deinit();

            var cqes: [256]std.os.linux.io_uring_cqe = undefined;
            while (self.writes.queued > 0) {
                ...
                for (cqes[0..n]) |cqe| {
                    ...
                    if (cqe.res >= 0) {
                        try self.submitEntry(idx, @intCast(cqe.res));
                        continue;
                    }

                    return switch (@as(posix.E, @enumFromInt(-cqe.res))) {
                        .INTR => {
                            try self.submitEntry(idx, 0);
                            continue;
                        },
                        ...
                        .SPIPE => error.Unseekable,
                            ...
                            std.debug.panic("ring.write: unexpected error {}", .{errno});
                        },
                    };
                }
            }
    ...
    fn submitEntry(self: *Disk, idx: u64, written: u64) !void {
        const entry = &self.getWriteMap().array[idx];
        entry.op.disk_offset += @intCast(written);
        entry.buf_offset += @intCast(written);
        entry.op.len -= @intCast(written);

        if (entry.op.len == 0) {
            self.writes.queued -= 1;
            return;
        }
        
        while (true) {
            _ = self.writes.ring.write(
                idx,
                self.file,
                self.getWriteBuffer()[entry.buf_offset..][0..entry.op.len],
                entry.op.disk_offset,
            ) catch {
                const flush_zone = tracy.Zone.init(@src(), .{ .name = "Disk.ringSubmit" });
                defer flush_zone.deinit();

                _ = try self.writes.ring.submit();
                continue;
            };
            break;
        }
