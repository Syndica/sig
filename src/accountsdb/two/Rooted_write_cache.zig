...
const Disk = struct {
    ...
    fn queueWrite(self: *Disk, op: WriteOp, aligned: u32) !WriteBufOffset {
        while (true) {
            ...
            self.writes.size = new_offset + op.len;
            self.writes.queued += 1;

            if (self.writes.size <= max_write_buf and self.writes.queued <= max_write_queue) {
                @branchHint(.likely);
                return new_offset;
            } else try self.flushWrites();
        }
    }
    ...
    fn flushWrites(self: *Disk) !void {
        ...
        for (&self.getWriteMap().array) |entry| {
            if (@as(u64, @bitCast(entry.op)) == 0) continue;
            const write_buf = self.getWriteBuffer()[entry.buf_offset..][0..entry.op.len];

            const write_zone = tracy.Zone.init(@src(), .{ .name = "Disk.pwrite" });
            defer write_zone.deinit();

            // TODO: io_uring
            var n: usize = 0;
            while (n < entry.op.len) {
                n += try posix.pwrite(self.file, write_buf[n..], entry.op.disk_offset + n);
            }
        } 
