...
fn shrinkAccountFiles(
    ...
    for (shrink_account_files) |shrink_file_id| {
        ...
        // update slot's reference memory
        {
            ...
            db.account_index.reference_manager.free(slot_reference_map_entry.value_ptr.items.ptr);
...
fn purgeSlot(db: *AccountsDB, slot: Slot) void {
    ...
    // free the reference memory
    {
        ...
        db.account_index.reference_manager.free(r.value.items.ptr);
    }
...
test "clean to shrink account file works with zero-lamports" {
    ...
    var account = try accounts_db.getAccountDeprecated(&pubkey_remain);
...
test "shrink account file works" {
    ...
    const account = try accounts_db.getAccountDeprecated(&pubkey_remain);
