...
pub const AccountsDB = struct {
    ...
    pub fn loadWithDefaults(
        ...
        const snapshot_manifest = try combined_manifest.collapse(self.allocator);
        errdefer snapshot_manifest.deinit(self.allocator);
        ...
        if (should_fastload) {
            ...
            try self.fastload(fastload_dir, snapshot_manifest.accounts_db_fields);
            self.logger.info().logf("loaded from snapshot in {s}", .{timer.read()});
        } else {
            const load_duration = try self.loadFromSnapshot(
                snapshot_manifest.accounts_db_fields,
                n_threads,
                allocator,
        ...
        if (validate) {
            const full_man = combined_manifest.full;
            const maybe_inc_persistence = if (combined_manifest.incremental) |inc|
...
fn loadTestAccountsDB(
    ...
    const manifest = try combined_manifest.collapse(allocator);
    ...
    return .{ accounts_db, combined_manifest };
}
...
test "geyser stream on load" {
    ...
    const snapshots = try FullAndIncrementalManifest.fromFiles(
        allocator,
        logger,
        snapdir,
        snapshot_files,
...
test "write and read an account" {
    ...
    var accounts_db, const snapshots = try loadTestAccountsDB(allocator, false, 1, .noop, snapdir);
    defer {
        accounts_db.deinit();
...
test "generate snapshot & update gossip snapshot hashes" {
    ...
    const full_slot = combined_manifest.full.accounts_db_fields.slot;
    const full_gen_result = try accounts_db.generateFullSnapshot(.{
        ...
        .deprecated_stored_meta_write_version = combined_manifest.full.accounts_db_fields.stored_meta_write_version,
    });
    const full_hash = full_gen_result.hash;

    try std.testing.expectEqual(
        combined_manifest.full.accounts_db_fields.bank_hash_info.accounts_hash,
        full_gen_result.hash,
    );
    try std.testing.expectEqual(
        combined_manifest.full.bank_fields.capitalization,
    ...
    if (combined_manifest.incremental) |inc_manifest| {
        ...
        try std.testing.expectEqual(inc_manifest.bank_extra.snapshot_persistence, inc_gen_result);
