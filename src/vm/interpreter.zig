...
pub fn Vm(Context: type) type {
    return struct {
        ...
        pub fn deinit(self: *Vm(Context)) void {
            self.call_frames.deinit(self.allocator);
        }

        pub fn run(self: *Vm(Context)) struct { Result, u64 } {
            const initial_instruction_count = self.context.getRemaining();
            while (true) {
        ...
        fn step(self: *Vm(Context)) SbpfError!bool {
            const config = self.executable.config;
            if (config.enable_instruction_meter and
                self.instruction_count >= self.context.getRemaining())
            {
...
pub const TestContextObject = struct {
    remaining: u64,
