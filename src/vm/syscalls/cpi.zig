...
fn cpiCommon(
    ...
    if (ids.NATIVE_LOADER_ID.equals(&instruction.program_id) or
        ...
        (bpf_loader_program.v3.ID.equals(&instruction.program_id) and !(blk: {
            // Check valid instruction type without verifying its contents.
            if (instruction.data.len == 0) break :blk false;
            const V3Tag = @typeInfo(bpf_loader_program.v3.Instruction).@"union".tag_type.?;
            const v3_tag = std.meta.intToEnum(V3Tag, instruction.data[0]) catch break :blk false;
            break :blk switch (v3_tag) {
                .close => true,
                .upgrade => true,
                .set_authority => true,
        ...
        // Check if precompile
        break :blk for (PRECOMPILES) |p| {
            if (p.program_id.equals(&instruction.program_id)) break true;
        } else false;
    })) {
