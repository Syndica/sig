...
fn parseStrict(
    ...
    // [agave] https://github.com/anza-xyz/sbpf/blob/v0.14.4/src/elf.rs#L427-L455
    ...
    // [agave] https://github.com/anza-xyz/sbpf/blob/v0.14.4/src/elf.rs#L457-L493
    // Expected program headers: rodata (PF_R) then bytecode (PF_X).
    // Note: In agave's SBPFv3, rodata is at vaddr 0x0 (MM_RODATA_START) and bytecode
    // at 0x100000000 (MM_BYTECODE_START). In sig's naming, these map to
    // BYTECODE_START (0x0) and RODATA_START (0x100000000) respectively.
    const expected_phdrs: [2]struct { u32, u64 } = .{
        .{ elf.PF_R, memory.BYTECODE_START }, // read only data (agave: MM_RODATA_START = 0x0)
        .{ elf.PF_X, memory.RODATA_START }, // byte code (agave: MM_BYTECODE_START = 0x100000000)
    };
    ...
    // [agave] https://github.com/anza-xyz/sbpf/blob/v0.14.4/src/elf.rs#L495-L523
    const bytecode_header = if (skip_rodata) phdrs[0] else phdrs[1];

    const ro_section: Section = if (skip_rodata)
        .{ .borrowed = .{
        ...
        .{ .borrowed = .{
            .offset = phdrs[0].p_vaddr,
            .start = phdrs[0].p_offset,
            .end = phdrs[0].p_offset +| phdrs[0].p_filesz,
        } };

    // [agave] https://github.com/anza-xyz/sbpf/blob/v0.14.4/src/elf.rs#L510-L518
