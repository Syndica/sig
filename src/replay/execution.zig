...
fn prepareSlot(
    ...
    const ref, var lock = slot_tracker.get(slot) orelse return error.MissingSlot;
    defer lock.unlock();

    const i_am_leader = ref.constants.collector_id.equals(&state.identity.validator);

    if (!progress_get_or_put.found_existing) {
        const parent_slot = ref.constants.parent_slot;
        ...
        progress_get_or_put.value_ptr.* = try ForkProgress.initFromParent(state.allocator, .{
            ...
            .parent = parent,
            .slot_hash = ref.state.hash.readCopy(),
            .last_entry = ref.state.blockhash_queue.readField("last_hash") orelse
    ...
    const tick_height =
        ref.state.tick_height.fetchAdd(core.entry.tickCount(entries), .monotonic);

    const new_rate_activation_epoch =
        if (ref.constants.feature_set.get(.reduce_stake_warmup_cooldown)) |active_slot|
            epoch_tracker.epoch_schedule.getEpoch(active_slot)
        else
            null;

    const slot_account_reader = state.account_store.reader()
        .forSlot(&ref.constants.ancestors);
    ...
    const resolved_txns = try replay.resolve_lookup.resolveBlock(state.allocator, entries, .{
        ...
        .reserved_accounts = &ref.constants.reserved_accounts,
    ...
    var svm_gateway = try SvmGateway.init(state.allocator, .{
        ...
        .lamports_per_signature = ref.constants.fee_rate_governor.lamports_per_signature,
        .blockhash_queue = &ref.state.blockhash_queue,
        .account_store = state.account_store.forSlot(slot, &ref.constants.ancestors),
        .ancestors = &ref.constants.ancestors,
        .feature_set = ref.constants.feature_set,
        .rent_collector = &ref.constants.rent_collector,
        .epoch_stakes = &epoch_info.stakes,
    ...
    const committer = replay.Committer{
        ...
        .slot_state = ref.state,
        .status_cache = &state.status_cache,
        .stakes_cache = &ref.state.stakes_cache,
    ...
    const verify_ticks_params = replay.execution.VerifyTicksParams{
        ...
        .max_tick_height = ref.constants.max_tick_height,
...
test "prepareSlot: empty and dead slots are handled correctly" {
    ...
    const root_ref, var root_lock = state.slot_tracker.get(0).?;
