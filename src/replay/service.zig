...
test "bypass-consensus slot updates emit tip_changed and rooted chain events" {
    const allocator = std.testing.allocator;
    ...
    const slot_update = replay_state.slot_tree.reRoot(allocator);
    try handleSlotUpdate(allocator, &replay_state, slot_update, null, metrics);

    const tip_event = event_sink.channel.tryReceive() orelse return error.TestUnexpectedResult;
    defer tip_event.deinit(allocator);
    switch (tip_event) {
    ...
    try std.testing.expectEqual(2, replay_state.slot_tracker.consensus_root.load(.monotonic));
    try std.testing.expectEqual(2, replay_state.slot_tracker.state_root.load(.monotonic));
    try std.testing.expect(replay_state.commitments == null);
    try std.testing.expect(event_sink.channel.tryReceive() == null);
}
