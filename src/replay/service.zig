...
pub fn advanceReplay(params: AdvanceReplayParams) !void {
    ...
    const logger = params.replay_state.logger;
    ...
    if (params.consensus) |consensus| {
        ...
        try runConsensus(allocator, logger, .{
...
pub fn runConsensus(
    ...
    if (params.vote_collection) |vp| {
        try vp.collector.collectAndProcessVotes(allocator, .from(logger), .{
            .slot_data_provider = .{
            ...
            .receivers = .{ .replay_votes = params.replay_state.replay_votes_channel },
            .ledger = params.replay_state.ledger,
            .gossip_table = vp.gossip_table,
        });
    }

    try params.tower.process(
        allocator,
...
test "process runs without error with no replay results" {
    ...
    try consensus.process(
        ...
        consensus_senders,
        consensus_receivers,
        vc_senders.duplicate_confirmed_slots.?.items,
        vc_senders.gossip_verified_vote_hashes.items,
        &.{},
    );
}
...
pub const DependencyStubs = struct {
    ...
    pub fn stubbedState(
        ...
        return try .init(.{
            .allocator = allocator,
            .logger = logger,
            .my_identity = .initRandom(prng),
            .vote_identity = .initRandom(prng),
