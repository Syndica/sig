...
fn getSysvarAndDataFromAccount(
    ...
    const sysvar = bincode.deserializeSlice(allocator, Sysvar, data, .{}) catch {
        allocator.free(data);
        return null;
    };
...
fn getSysvarFromAccount(
    ...
    return bincode.deserializeAlloc(allocator, Sysvar, data.reader(), .{}) catch return null;
}
...
fn getSysvarAndAccount(
    ...
    const data = try account.data.readAllAllocate(allocator);
    const sysvar = bincode.deserializeSlice(
        allocator,
        Sysvar,
        data,
        .{},
