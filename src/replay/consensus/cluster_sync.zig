    ...
    while (ancestor_duplicate_slots_receiver.tryReceive()) |ancestor_dupe_slot_to_repair| {
        ...
        const slot_status: SlotStatus = status: {
            ...
            break :status .fromHash(hash: {
                ...
                const slot_info =
                    slots_lg.get().get(epoch_slots_frozen_slot) orelse
                    break :hash null;
    ...
    for (duplicate_confirmed_slots_received) |new_duplicate_confirmed_slot| {
        ...
        const duplicate_confirmed_state: DuplicateConfirmedState = .{
            ...
            .slot_status = status: {
                ...
                const slot_hash = if (slot_tracker.get(confirmed_slot)) |ref| blk: {
                    defer ref.release();
    ...
    const root_slot, const slots_hashes = blk: {
        ...
        for (new_duplicate_slots.constSlice()) |duplicate_slot| {
            slots_hashes.appendAssumeCapacity(hash: {
                var slots_lg = slot_tracker.slots.read();
                defer slots_lg.unlock();
                const bf_elem = slots_lg.get().get(duplicate_slot) orelse break :hash null;
    ...
    for ([_][]const Slot{
        ...
        for (child_slot_set) |child_slot| {
            const child_hash = blk: {
                const ref = slot_tracker.get(child_slot).?;
                defer ref.release();
                break :blk ref.state().hash.readCopy().?;
            };
...
test "check slot agrees with cluster dead duplicate confirmed" {
    ...
    const slot2_hash = blk: {
        const ref = slot_tracker.get(2).?;
        defer ref.release();
        break :blk ref.state().hash.readCopy().?;
    };
...
fn testStateDuplicateThenSlotFrozen(initial_slot_hash: ?Hash) !void {
    ...
    for (2..3 + 1) |slot| {
        const slot_hash = blk: {
            const ref = slot_tracker.get(slot).?;
            defer ref.release();
            break :blk ref.state().hash.readCopy().?;
        };
    ...
    // Now freeze the slot
    const frozen_duplicate_slot_hash = blk: {
        const ref = slot_tracker.get(duplicate_slot).?;
        defer ref.release();
        break :blk ref.state().hash.readCopy().?;
    };
...
test "state ancestor confirmed descendant duplicate" {
    ...
    const slot3_hash = blk: {
        const ref = slot_tracker.get(3).?;
        defer ref.release();
        break :blk ref.state().hash.readCopy().?;
    };
    try std.testing.expectEqual(
