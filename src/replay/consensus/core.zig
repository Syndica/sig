...
pub const TowerConsensus = struct {
    ...
    ///
    /// Returns the new root, if it changed.
    pub fn process(
        self: *TowerConsensus,
        allocator: Allocator,
        params: struct {
    ...
    fn executeProtocol(
        ...
        if (maybe_voted_slot) |voted| {
            ...
            maybe_new_root = try handleVotableBank(
                self.logger,
                allocator,
                voted.slot,
                voted_hash,
...
fn loadTower(
    ...
    const vote_state = try versioned_state.convertToVoteState(allocator, null);
...
fn handleVotableBank(
    ...
    const maybe_new_root = try replay_tower.recordBankVote(allocator, vote_slot, vote_hash);
    ...
    if (maybe_new_root) |new_root| {
        ...
        try fork_choice.setTreeRoot(allocator, &.{ .slot = new_root, .hash = root_hash });
    }
...
fn generateVoteTx(
    ...
    var vote_state = vote_state_versions.convertToVoteState(allocator, null) catch |err| {
