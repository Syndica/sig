...
pub const StreamReader = struct {
    ...
    pub fn readPayload(self: *Self) !Payload.Data {
...
test "streaming accounts" {
    ...
    // setup writer
    var stream_writer = try StreamWriter.init(allocator, "test_data/stream_test.pipe");
    defer stream_writer.deinit();

    // setup reader
    var stream_reader = try StreamReader.init(allocator, "test_data/stream_test.pipe");
    defer stream_reader.deinit();

    // write to the pipe
    try stream_writer.streamBatch(100, accounts, pubkeys);

    // read from the pipe
    const data = try stream_reader.readPayload();
    ...
    // #2
    try stream_writer.streamBatch(100, accounts2, pubkeys2);

    // first payload matches
    const data2 = try stream_reader.readPayload();
    ...
    const data3 = try stream_reader.readPayload();
...
pub const BenchmarkAccountStream = struct {
    ...
    pub fn benchmarkAccountStreaming(bench_args: BenchmarkArgs) !u64 {
        ...
        // setup writer
        var stream_writer = try StreamWriter.init(allocator, "test_data/bench_test.pipe");
        defer stream_writer.deinit();

        // setup reader
        const stream_reader = try allocator.create(StreamReader);
        stream_reader.* = try StreamReader.init(allocator, "test_data/bench_test.pipe");
        defer {
        ...
        for (0..bench_args.slots) |slot| {
            try stream_writer.streamBatch(slot, accounts, pubkeys);
        }
...
pub fn readLoop(stream_reader: *StreamReader, n_payloads: usize) !void {
    var count: usize = 0;
