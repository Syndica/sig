...
/// `LogCollector` is used to collect logs at the transaction level. Each
/// `TransactionContext` has its own log collector which may be used to collect
/// and emit logs as part of the transaction processing result.
///
