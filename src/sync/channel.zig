...
pub fn Channel(T: type) type {
    return struct {
        ...
        // NOTE: if we start seeing performance problems with the channel implementation in the
        // future
        // note that it's possible to pre-allocate an initial capacity of blocks in order to speed
        // it up.
            ...
            while (true) {
                ...
                if (channel.tail.index.cmpxchgWeak(tail, new_tail, .seq_cst, .acquire)) |t| {
                    ...
                    if (offset + 1 == BLOCK_CAP) {
                        ...
                        // tail.index store above.
                        // Moving allocation after the CAS increases the amount of time other
                        // threads must snooze.
                        // Moving the allocation before the CAS lowers it, but you need to handle
                        // the install
        ...
        pub fn tryReceive(channel: *Self) ?T {
            ...
            while (true) {
                ...
                if (new_head & HAS_NEXT == 0) {
                    // NOTE: could also be tail.index.load(.acquire), but to be safe, seq_cst RMW
                    // load
                    const tail = channel.tail.index.fetchAdd(0, .seq_cst);

                    // If the indicies are the same, the channel is empty and there's nothing to
                    // receive.
                    ...
                    // Using this invariance, we can prove that if the head is in a different block
                    // than
        ...
        pub fn len(channel: *Self) usize {
            while (true) {
                ...
                if (channel.tail.index.load(.seq_cst) == tail) {
                    ...
                    // Calculate on which block link we're on. Between 0-31 is block 1, 32-63 is
                    // block 2, etc.
