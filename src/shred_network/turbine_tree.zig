...
const SamplerTree = sig.random.weighted_shuffle.SamplerTree(.shift);

/// Cache turbine trees and clear them once they are too old.
/// The time to live ensures updates to gossip data are reflected in the turbine trees.
pub const TurbineTreeCache = struct {
    cache: std.AutoArrayHashMapUnmanaged(Epoch, Entry),

    const time_to_live: Duration = .fromSecs(5);
    pub const empty: TurbineTreeCache = .{ .cache = .empty };

    pub const Entry = struct {
        created: Instant,
        turbine_tree: *TurbineTree,

        pub fn isAlive(self: *const Entry) bool {
            const elapsed = self.created.elapsed(); // time elapsed since the stored instance
            return elapsed.lt(time_to_live); // entry is alive if it has not existed longer than the TTL
        }
    };

    pub fn deinit(self: *TurbineTreeCache, allocator: std.mem.Allocator) void {
    ...
    pub fn get(self: *TurbineTreeCache, epoch: Epoch) !?*TurbineTree {
        if (self.cache.getPtr(epoch)) |entry| {
            if (entry.isAlive()) {
                return entry.turbine_tree.acquireUnsafe();
            } else {
                // de-init the tree if its entry in the cache is dead.
    ...
    pub fn put(
        ...
        try self.cache.put(allocator, epoch, .{
            .created = Instant.now(),
            .turbine_tree = turbine_tree.acquireUnsafe(),
        });

        // TODO: we can potentially sort the turbine tree cache by time here, iterating through it
        // backwards, from the oldest to the newest entries, and top removing entries after the
        // last expired one, since in theory, the entries will all have monotonic clocks.

        for (self.cache.keys(), self.cache.values()) |key, entry| {
            if (!entry.isAlive()) {
                entry.turbine_tree.releaseUnsafe();
                std.debug.assert(self.cache.swapRemove(key));
            }
        }
    }
};
...
pub const TurbineTree = struct {
    ...
    pub fn initForRetransmit(
        ...
        var weighted_shuffle: SamplerTree = .init(nodes.items.len, @splat(0));
        errdefer weighted_shuffle.deinit();

        for (nodes.items, 0..) |node, i| {
            try index.put(node.pubkey(), i);
            weighted_shuffle.addWeight(node.stake);
        }
    ...
    ) !u64 // root distance
    {
        ...
        var weighted_shuffle = self.weighted_shuffle;
        if (self.index.get(slot_leader)) |index| {
            weighted_shuffle.remove(index);
        }
        weighted_shuffle.rng = getSeededRng(slot_leader, shred_id); // update the rng for this case

        // Shuffle the nodes and find my index
        var maybe_my_index: ?u64 = null;

        try shuffled_nodes.ensureTotalCapacity(self.nodes.items.len);
        while (weighted_shuffle.sampleAndRemove()) |index| {
    ...
    fn getSeededRng(leader: Pubkey, shred_id: ShredId) ChaCha {
        ...
        return .init(hasher.finalResult());
    }
