...
test "RepairService sends repair request to gossip peer" {
    ...
    // assertions
    try std.testing.expectEqual(160, peer_socket.receive(&buf));
    const msg = try bincode.readFromSlice(allocator, RepairMessage, buf[0..160], .{});
    try msg.verify(buf[0..160], .fromPublicKey(&peer_keypair.public_key), @intCast(std.time.milliTimestamp()));
    try std.testing.expectEqual(13579, msg.highest_window_index.slot);
