...
test "handleDuplicateSlots: no sender configured" {
    ...
    try shred_receiver.params.duplicate_handler.handleDuplicateSlots(allocator, &result);
}
...
test "handleDuplicateSlots: emits and stores via handleDuplicateSlot" {
    ...
    var conflict_payload = try allocator.dupe(u8, original.payload());
...
test "pushDuplicateShredToGossip: enqueues chunks and ring indices" {
    ...
    const proof_bytes = try shred_network.duplicate_shred_handler.serializeDuplicateProof(
        allocator,
        shred_payload,
        other_payload,
    );
...
test "computeRingOffset: under capacity and at capacity oldest index" {
    ...
    const offset_under =
        shred_network.duplicate_shred_handler.computeRingOffset(
