...
/// Retransmit Service
/// The retransmit service receives verified shreds from the shred
/// network and retransmits them to the network.
/// The retransmit service is broken down into two main components:
/// 1. receiveShreds: runs on a single thread and receives shreds
///    from the shred network, deduplicates them, and then packages
///    them into RetransmitShredInfo's which are sent to a channel
///    for further processing.
/// 2. retransmitShreds: runs on N threads and receives
///    RetransmitShredInfo's from the channel, computes the children
///    to retransmit to, and then constructs and sends packets to
///    the network.
...
/// turbine_tree
/// The shred_id and slot_leader are used to seed an rng for shuffling the nodes in the turbine_tree
/// before
