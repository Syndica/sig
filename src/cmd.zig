...
fn identity(gpa_allocator: std.mem.Allocator, cfg: config.Cmd) !void {
    const logger = try spawnLogger(gpa_allocator, cfg);
    defer logger.deinit();
...
const AppBase = struct {
    ...
    fn init(allocator: std.mem.Allocator, cfg: config.Cmd) !AppBase {
        const logger = (try spawnLogger(allocator, cfg)).withScope(LOG_SCOPE);
        errdefer logger.deinit();

        const metrics_registry = globalRegistry();
        const metrics_thread = try spawnMetrics(allocator, cfg.metrics_port);
        ...
        // zig fmt: off
        const my_shred_version = cfg.shred_version
...
fn spawnLogger(allocator: std.mem.Allocator, cfg: config.Cmd) !Logger {
