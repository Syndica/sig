...
pub fn main() !void {
    ...
    const shred_version_option: mcli.OptionInfo(?u16) = .{
    ...
    const leader_schedule_option: mcli.OptionInfo(?[]const u8) = .{
    ...
    const gossip_network_option: mcli.OptionInfo(?[]const u8) = .{
    ...
    const snapshot_dir_option: mcli.OptionInfo([]const u8) = .{
    ...
    const genesis_file_path_option: mcli.OptionInfo(?[]const u8) = .{
    ...
    const GossipOptionsCommon = struct {
        ...
        const cmd_info: mcli.OptionInfoGroup(@This()) = .{
            .host = .{
        ...
        fn apply(opts: @This(), cfg: *config.Cmd) void {
            ...
            cfg.gossip.network = opts.network;
        }
    };
    ...
    const Cmd = struct {
        ...
        const cmd_info: mcli.CommandInfo(@This()) = .{
            .help = .{
                .short =
        ...
        const identity_cmd_info: mcli.CommandInfo(void) = .{
            .help = .{
    ...
    const argv = try std.process.argsAlloc(gpa_allocator);
    defer std.process.argsFree(gpa_allocator, argv);

    const parser = mcli.Parser(Cmd, Cmd.cmd_info);
    ...
    defer parser.free(gpa_allocator, cmd);
    ...
    switch (cmd.subcmd) {
        ...
        .validator => |params| {
            ...
            current_config.accounts_db.force_new_snapshot_download =
                params.force_new_snapshot_download;
        ...
        .snapshot_validate => |params| {
            ...
            params.accountsdb_base.apply(&current_config);
            params.accountsdb_index.apply(&current_config);
            current_config.gossip.network = params.gossip_network;
            params.geyser.apply(&current_config);
        ...
        .test_transaction_sender => |params| {
            ...
            params.gossip_base.apply(&current_config);
            params.gossip_node.apply(&current_config);
            try testTransactionSenderService();
        },
