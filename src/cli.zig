...
pub fn CommandInfo(comptime S: type) type {
    ...
    return struct {
        ...
        pub const Cmd = S;
        pub const SubInfo = Sub;
        pub const option_count = opt_count;
    };
}
...
pub fn Parser(
    ...
    return struct {
        ...
        pub fn free(allocator: std.mem.Allocator, cmd: Cmd) void {
            helper.freeCmdImpl(allocator, cmd);
        }
    };
}
...
fn ParserTester(
    ...
    return struct {
        ...
        fn expectHelp(
            ...
            } else std.debug.panic("Missing expected `-h`/`--help` argument in `args`: {s}", .{args});
...
test "TestCmd" {
    const TestCmd = if (@import("builtin").is_test) struct {
...
fn optCountAndSubInfo(comptime T: type) struct { usize, type } {
    const s_info = switch (@typeInfo(T)) {
        .Struct => |s_info| s_info,
        .Void => return .{ 0, struct {} },
        else => unreachable,
    };

    var opt_count: usize = 0;
    const Type = std.builtin.Type;
    var fields: [s_info.fields.len]Type.StructField = undefined;

    @setEvalBranchQuota(s_info.fields.len * 2 + 1);
    for (&fields, s_info.fields) |*new_s_field, s_field| {
...
fn CmdHelper(
    ...
    const OptEnumIntMax = std.math.IntFittingRange(0, CmdInfo.option_count);
    ...
    const alias_table_wip: [MAX_ALIAS_TABLE_LEN]OptEnumIntMax, //
    // partially default-initialised result
    const default_init: Cmd, //
    const maybe_sub_cmd_s_field_index: ?usize //
    = blk: {
        var opt_enum_fields: []const Type.EnumField = &.{};
        var opt_enum_to_field_map: []const OptStructIndex = &.{};

        var alias_table_wip = [_]OptEnumIntMax{cmd_fields.len} ** MAX_ALIAS_TABLE_LEN;
                ...
                @typeInfo(@typeInfo(s_field.type).Optional.child) == .Union))
            {
                if (maybe_sub_cmd_s_field_index) |prev| @compileError(
                    "Cannot have two sub-command union fields in " ++ parent_name ++ ": " ++
                        cmd_fields[prev].name ++ " & " ++ s_field.name,
                );
    ...
    = alias_info: {
        ...
        const OptEnumIntMaxVec = @Vector(MAX_ALIAS_TABLE_LEN, OptEnumIntMax);
        const old_sentinel_vec: OptEnumIntMaxVec = @splat(cmd_fields.len);
        const used_aliases_mask_vec = alias_table_wip != old_sentinel_vec;
    ...
    const OptEnumIntPlusOne = std.math.IntFittingRange(0, opt_enum_fields.len);
    const alias_table_sentinel: OptEnumIntPlusOne = opt_enum_fields.len;
    const alias_table: [alias_table_len]OptEnumIntPlusOne = blk: {
        if (alias_table_len == 0) break :blk .{};

        const OptEnumIntMaxVec = @Vector(alias_table_len, OptEnumIntMax);
        const old_sentinel_vec: OptEnumIntMaxVec = @splat(cmd_fields.len);
        const new_sentinel_vec: OptEnumIntMaxVec = @splat(alias_table_sentinel);

        var alias_table_vec: OptEnumIntMaxVec = alias_table_wip[0..alias_table_len].*;
        alias_table_vec = @select(
            OptEnumIntMax,
            alias_table_vec != old_sentinel_vec,
            alias_table_vec,
            new_sentinel_vec,
        );
        break :blk alias_table_vec;
    };

    return struct {
        fn freeCmdImpl(allocator: std.mem.Allocator, args: Cmd) void {
            ...
            switch (cmd_field_value) {
                inline else => |payload, itag| {
                    ...
                    const sub_parent = parent_prefix ++ sub_cmd_s_field_name ++ "." ++ @tagName(itag);
                    const sub_helper = CmdHelper(@TypeOf(payload), sub_cmd_info, sub_parent);
                    sub_helper.freeCmdImpl(allocator, payload);
                },
            }
        }
        ...
        fn parseCmdInner(
            ...
            const subcmd_set: bool = while (args_iter.next()) |arg| {
                ...
                parse_opt: {
                    ...
                    switch (opt_tag) {
                        inline else => |itag| {
                            ...
                            const is_list, const ListElem = switch (@typeInfo(Opt)) {
                                .Pointer => |ptr_info| blk: {
                                    const is_list = ptr_info.size == .Slice and
                                        (ptr_info.child != u8 or opt_info.config == .list);
                                    const ListElem = if (is_list) ptr_info.child else {};
                                    break :blk .{ is_list, ListElem };
                                },
                                else => .{ false, {} },
                            };
                            if (!is_list) {
                                opt_ptr.* = try parseSingleOptValueMaybeScan(
                                    opt_name,
                                    Opt,
                                    maybe_value,
                                    args_iter,
                                ...
                                const parsed = try parseSingleOptValueMaybeScan(
                                    opt_name,
                                    ListElem,
                                    maybe_value,
                                    args_iter,
                                ...
                                @constCast(&opt_ptr.*[opt_ptr.len - 1]).* = parsed;
                            }
                        },
                    }
                ...
                switch (cmd_tag) {
                    inline else => |itag| {
                        ...
                        subcmd_ptr.* = try sub_helper.parseCmdInner(
                            allocator,
            ...
            if (subcmd_set) {
                comptime std.debug.assert(maybe_sub_cmd_s_field_index != null);
            } else if (maybe_sub_cmd_s_field_index) |sub_cmd_s_field_index| {
                const sub_cmd_s_field_info = cmd_fields[sub_cmd_s_field_index];
                const sub_cmd_s_field_name = sub_cmd_s_field_info.name;
                const CmdMaybeUnion = sub_cmd_s_field_info.type;
        ...
        fn writeHelp(
            ...
            if (maybe_sub_cmd_s_field_index) |sub_cmd_s_field_index| {
                const SubCmdNameHelpPair = struct { []const u8, []const u8 };
                const subcmd_list: []const SubCmdNameHelpPair = comptime blk: {
                    const sub_cmd_s_field_info = cmd_fields[sub_cmd_s_field_index];
                    const sub_cmd_s_field_name = sub_cmd_s_field_info.name;
                    const CmdMaybeUnion = sub_cmd_s_field_info.type;
...
fn parseSingleOptValue(
    ...
    switch (@typeInfo(T)) {
        ...
        .Optional => |optional| if (@typeInfo(optional.child) != .Optional) {
            ...
            return try parseSingleOptValue(option_name, optional.child, value_str);
        },
        else => {},
    }
...
inline fn constEql(comptime a: []const u8, b: []const u8) bool {
    if (a.len != b.len) return false;
    const a_vec: @Vector(a.len, u8) = a[0..a.len].*;
    const b_vec: @Vector(a.len, u8) = b[0..a.len].*;
