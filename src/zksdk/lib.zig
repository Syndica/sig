...
/// The scalar `1`
const ONE: Edwards25519.scalar.CompressedScalar = .{1} ++ .{0} ** 31;
...
pub const ZeroCiphertextProof = struct {
    ...
    pub fn init(
        ...
        ciphertext: *const el_gamal.Ciphertext,
    ...
    pub fn verify(
        ...
        transcript.appendMessage("Y_D", &self.D.toBytes());
    ...
    test "edge case" {
...

        {
            ...
            var ciphertext = try el_gamal.Ciphertext.fromBytes(.{0} ** 64);
...

        {
            ...
            const zeroed_commitment: el_gamal.pedersen.Commitment = .{
                .point = try Ristretto255.fromBytes(.{0} ** 32),
            };
            const opening = el_gamal.pedersen.Opening.random();
            const handle = el_gamal.pedersen.DecryptHandle.init(&kp.public, &opening);

            const ciphertext: el_gamal.Ciphertext = .{
...

        {
            ...
            const commitment, _ = el_gamal.pedersen.init(u64, 0);
            const ciphertext: el_gamal.Ciphertext = .{
        ...
        // if the public key is zeroed, then the proof should always reject
        {
            ...
            const public: el_gamal.Pubkey = .{ .p = try Ristretto255.fromBytes(.{0} ** 32) };
    ...
    test "proof string" {
        ...
        const pubkey = try el_gamal.Pubkey.fromBase64(pubkey_string);

        // zig fmt: off
        const ciphertext_string = "wps5X1mou5PUdPD+llxiJ+aoX4YWrR/S6/U2MUC2LjLS7wDu6S9nOG92VMnlngQaP4irBY0OqlsGdXS4j8DROg==";
        const ciphertext = try el_gamal.Ciphertext.fromBase64(ciphertext_string);
