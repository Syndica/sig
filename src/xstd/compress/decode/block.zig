...
pub fn decodeBlockReader(
    dest: *RingBuffer, // written to
    source: anytype,
    block_header: frame.Zstandard.Block.Header,
    decode_state: *DecodeState, // this is written to
    block_size_max: usize,
    literals_buffer: []u8,
    sequence_buffer: []u8,
) !void {
    ...
    switch (block_header.block_type) {
        .raw => {
            ...
            var block_reader = block_reader_limited.reader();

            const slice = dest.sliceAt(dest.write_index, block_size);
            try block_reader.readNoEof(slice.first);
            try block_reader.readNoEof(slice.second);
            dest.write_index = dest.mask2(dest.write_index + block_size);
            decode_state.written_count += block_size;
        },
        .rle => {
            var block_reader_limited = std.io.limitedReader(source, 1);
            var block_reader = block_reader_limited.reader();

            const byte = try block_reader.readByte();
            for (0..block_size) |_| {
