...
zig-out/bin/sig validator -n testnet --snapshot-metadata-only --overwrite-stake-for-testing --test-repair-for-slot $(solana -ut slot) 2>&1 | tee demo/turbine-retransmit-demo.log | grep demo.
