        ...
        .ok => |txn| {
            ...
            const converted = blk: {
                ...
                const txn_error: TransactionError = switch (txn) {
                    .executed => |executed| if (executed.executed_transaction.instr_err) |err|
                        .{ .InstructionError = .{ err[0], err[1] } }
                    else
                        break :blk utils.ConvertedErrorCodes.default,
                ...
                if (converted.instruction_index < sanitized.instructions.len) {
                    const ixn_info = &sanitized.instructions[converted.instruction_index];
